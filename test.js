...
/*
 * EL CUERPO DE UNA FUNCION, POR LLAVES BALANCEADAS. Usalo en vez de cortar N caracteres.
 *
 * Las ventanas de tamaño fijo son el error mas repetido de este archivo: 400 caracteres en la
 * guarda del ojito (se paso a la funcion de al lado y fallo sobre codigo correcto), 900 en la
 * de secuencia (un comentario nuevo empujo el mensaje afuera), 3000 en la de la traba, y 14000
 * en la del resumen de `exportar` — esta ultima rota el 2026-09-21 por agregarle a `exportar`
 * un bloque de 1.400 caracteres que no tenia nada que ver.
 *
 * Una ventana mide el LARGO del bloque y no su contenido, asi que documentar mejor una rama, o
 * agregar una funcion al lado, rompe la guarda. Y romper una guarda con codigo correcto es el
 * peor modo de fallo que puede tener.
 */
...
/* id y shortname identifican al plugin para Premiere: si coinciden con los de OTRO
   plugin UXP, Premiere los trata como el MISMO y uno pisa al otro. Por eso el `id` del
   manifest hay que cambiarlo por uno propio antes de instalar.

   Antes esto comparaba contra UN plugin concreto leido de una ruta del Desktop. Dos
   problemas: si ese archivo no estaba, el chequeo informaba "no se puede comparar" y
   pasaba —o sea no cubria nada— y no miraba ningun OTRO plugin instalado. Ahora recorre
   la carpeta de plugins externos, que es donde Premiere los busca de verdad. */
const EXTERNOS = path.join(require("os").homedir(),
  "Library/Application Support/Adobe/UXP/Plugins/External");
const choquesManifest = [];
if (fs.existsSync(EXTERNOS)) {
  for (const d of fs.readdirSync(EXTERNOS)) {
    ...
    if (otro.id === manifest.id || otro.shortname === manifest.shortname) choquesManifest.push(d);
  }
}
if (choquesManifest.length) {
  mal("el manifest colisiona con otro plugin UXP instalado: " + choquesManifest.join(", "),
 ...
 */
{
  ...
  /* Y los del README, si los declara. En el repo PUBLICO quedaron en 51 sobre 65 con el código
   * en 54 sobre 70, y nadie lo vio hasta que lo leyó alguien en GitHub: este chequeo miraba solo
   * el CLAUDE.md (2026-09-23). Un README que no los declara no tiene nada que envejecer. */
 ...
 * guardó el proyecto de una agencia. Guardar es inocuo; un `borrar` con la misma confusión no lo sería.
 *
 ...
 */
{
  ...
  for (const guarda of ["proyecto", "secuencia"]) {
    ...
    for (const b of bloques) {
      /* El schema se lee por LLAVES BALANCEADAS y tolerando un envoltorio.
       *
       * Antes anclaba en `inputSchema: {` y terminaba en el literal `\n    }`. El 2026-09-21 se
       * envolvieron los 55 schemas en `soloEstas({...})` —para que zod REBOTE una clave
       * inventada en vez de descartarla— y esta guarda, que es correcta, empezo a informar las
       * 55 como si no declararan nada. Rechazar codigo correcto es el peor modo de fallo, y la
       * causa fue medir por una forma fija en vez de por estructura. */
      const iSchema = b.cuerpo.indexOf("inputSchema:");
      let cuerpoSchema = null;
      if (iSchema !== -1) {
        const abre = b.cuerpo.indexOf("{", iSchema);
        if (abre !== -1) {
          let n = 0;
          for (let k = abre; k < b.cuerpo.length; k++) {
            if (b.cuerpo[k] === "{") n++;
            else if (b.cuerpo[k] === "}" && --n === 0) { cuerpoSchema = b.cuerpo.slice(abre + 1, k); break; }
          }
        }
      }
      if (!cuerpoSchema || !reGuarda.test(cuerpoSchema)) { sinDeclarar.push(b.nombre); continue; }
...
for (const h of ["leer.js", "armar.js", "verificar.js", "suplentes.js", "construir.js",
  ...
  } else if (/Cliente_\d{2}-\d{2}-\d{2}_/.test(src)) {
 ...
 */
{
  ...
  /*
   * Y el testigo del OVERWRITE EXACTO tambien en la pista de AUDIO.
   *
   * Un medio sin video que pisa exacto a otro audio del mismo largo no hace crecer ninguna
   * pista: sin este testigo el verbo contestaba "NO SE PUSO NADA" con el clip nuevo adentro
   * (medido el 2026-09-22, dos veces). Se exige por ESTRUCTURA y sin comentarios, porque este
   * archivo ya tuvo cinco guardas que matchearon su propia prosa:
   *   - el nombre de ANTES se lee de la lista de audio de ANTES y antes del overwrite,
   *   - `cambioEnPuntoA` compara lo que quedo en ese segundo contra ESE nombre (no contra el
   *     de video, ni contra una constante),
   *   - y `entro` lo usa.
   */
  {
    const limpio = cuerpo.replace(/\/\*[\s\S]*?\*\//g, "").replace(/^\s*\/\/.*$/gm, "");
    const defCambioA = (limpio.match(/const cambioEnPuntoA\s*=([^;]*);/) || [, ""])[1];
    const lectura = limpio.search(/nombreAntesEnPuntoA\s*=\s*String\(\s*await\s+itemsAntesA\[/);
    const overwrite = limpio.search(/createOverwriteItemAction\s*\(/);
    const defEntroL = (limpio.match(/const entro\s*=([^;]*);/) || [, ""])[1];
    const comparaBien = /puestoA\[0\]\.nombre\s*!==\s*nombreAntesEnPuntoA\b/.test(defCambioA);
    if (lectura === -1 || overwrite === -1 || lectura > overwrite) {
      mal("`insertar` no lee ANTES del overwrite qué clip de audio había en ese segundo",
        "sin el nombre de antes, un audio que pisa exacto a otro no se distingue de uno que no entró");
    } else if (!comparaBien) {
      mal("`cambioEnPuntoA` de `insertar` no compara el clip de audio de ahora contra el de ANTES",
        "tiene que ser `puestoA[0].nombre !== nombreAntesEnPuntoA`: comparar contra el de video o contra una constante no distingue el reemplazo del fantasma");
    } else if (!/\bcambioEnPuntoA\b/.test(defEntroL)) {
      mal("el `entro` de `insertar` no usa el testigo de audio",
        "un .wav que pisa exacto a otro del mismo largo informa \"NO SE PUSO NADA\" habiendo entrado");
    } else {
      ok("`insertar` reconoce el overwrite exacto también en la pista de audio");
    }
  }
   ...
   * entrar— lo detecta el verbo desde el 2026-09-10, y `colocar_sincro.js` y
   * `colocar_propuesta.js` NUNCA lo leyeron: validaban con `/quedó/`, y el resumen dice
   * "quedó en V…" apenas hay un clip en ese segundo, fantasma incluido. O sea que el arreglo
   * que se habia escrito POR esas herramientas no las protegia. Medido el 2026-09-22.
   *
   * Por POSICION y sin comentarios: entre cada `enviar("insertar"` y el primer verbo que
   * edita un clip despues —antes del siguiente insertar— tiene que haber un
   * `if (X.entro !== true)` o `if (!X.entro)` sobre la variable de la respuesta. Los que
   * insertan y solo releen (locucion, musica, sonido) no editan nada: no hay clip viejo que
   * puedan pisar, y exigirles la guarda seria rechazar uso correcto.
   */
  {
    const EDITAN = /enviar\(\s*"(editar|borrar|cortar|fijar|desactivar|renombrar|keyframe|escalaFija)"/g;
    const faltan = [];
    let revisadas = 0;
    const dirH = path.join(raiz, "herramientas");
    for (const f of fs.readdirSync(dirH).filter((x) => x.endsWith(".js"))) {
      const src = fs.readFileSync(path.join(dirH, f), "utf8")
        .replace(/\/\*[\s\S]*?\*\//g, "").replace(/^\s*\/\/.*$/gm, "");
      const ins = [...src.matchAll(/(\w+)\s*=\s*await\s+enviar\(\s*"insertar"/g)];
      for (let k = 0; k < ins.length; k++) {
        const p = ins[k].index, v = ins[k][1];
        const tope = k + 1 < ins.length ? ins[k + 1].index : src.length;
        EDITAN.lastIndex = p;
        const e = EDITAN.exec(src);
        if (!e || e.index > tope) continue;          // no edita un clip despues: nada que proteger
        revisadas++;
        const puerta = new RegExp(`if\\s*\\(\\s*(?:!\\s*${v}\\.entro|${v}\\.entro\\s*!==\\s*true)\\s*\\)`);
        if (!puerta.test(src.slice(p, e.index))) {
          faltan.push(`${f}: la respuesta \`${v}\` de insertar llega a \`${e[1]}\` sin mirar \`${v}.entro\``);
        }
      }
    }
    if (!revisadas) {
      mal("no encontré ninguna herramienta que inserte y después edite",
        "la guarda no está mirando nada: o se renombraron los verbos, o la regex dejó de ver las llamadas");
    } else if (faltan.length) {
      mal("hay herramientas que editan el clip recién insertado sin leer el veredicto de `insertar`",
        faltan.join(" | ") + " — el resumen dice \"quedó\" también en el FANTASMA, y los `editar` recortan el clip VIEJO");
    } else {
      ok(`las ${revisadas} llamadas a \`insertar\` seguidas de una edición deciden por \`entro\``);
    }
  }
 ...
 */
{
  ...
  if (desde === -1) {
    ...
    /* Un catch que no hace NADA con el error. El de `isDisabled` es a propósito y lleva
       ...
       OJO CON EL ALCANCE: el corte va de `radiografia` hasta `const VERBOS`, así que
       cubre TODOS los verbos definidos en el medio, no sólo `radiografia`. Eso es más
       de lo que el título dice y resultó útil —agarró un catch vacío en un verbo nuevo—
       pero el mensaje tiene que decir DÓNDE está, o manda a mirar el verbo equivocado. */
    const catches = cuerpo.match(/catch\s*\([^)]*\)\s*\{[^}]*\}/g) || [];
    for (const c of catches) {
      const adentro = c.slice(c.indexOf("{") + 1, -1).trim();
      if (adentro === "") {
        const pos = cuerpo.indexOf(c);
        const antes = cuerpo.slice(0, pos);
        const fn = [...antes.matchAll(/(?:async )?function (\w+)\(/g)].pop();
        problemas.push("hay un catch completamente vacío en `" + (fn ? fn[1] : "radiografia") + "`");
      }
    }
 ...
 */
{
  ...
  if (iP !== -1) {
    /* SIN COMENTARIOS antes de cortar: la ventana media PROSA, asi que documentar mejor la
       guarda la hacia fallar. Es el "chequear presencia no es chequear uso" de este archivo
       dado vuelta — un chequeo que depende de cuanto se escribio al lado. */
    const sinCom = srcComandos.replace(/\/\*[\s\S]*?\*\//g, "").replace(/\/\/[^\n]*/g, "");
    const jP = sinCom.indexOf("p.proyecto === \"string\"");
    const bloque = jP === -1 ? "" : sinCom.slice(jP, jP + 1400);
 ...
 *
 * Este chequeo AVISA si aparece uno nuevo sin tope, y para eso los conocidos van declarados con
 * su razón.
 *
 * `escalaFija` estaba declarado como "sin tope y se deja así: a la escala real hace 20 a 90
 * lecturas, muy por debajo de 176". **Ese número se midió sobre otro material**, y el 2026-09-14
 * 32 clips de 3840x2160 a 50fps mataron Premiere donde 35 verticales habían pasado: la variable
 * es el PESO, no el conteo, así que ninguna cota en clips vale para todo. Ahora tiene `limite`
 * (30) y `siguiente`, como los otros tres, y el chequeo lo EXIGE.
 */
{
  const CONOCIDOS = {
    // Ya no es la excepción: tiene limite (30) y siguiente. Ver el chequeo aparte más abajo.
    escalaFija: "tiene limite (30) y siguiente; 1 lectura por clip",
 ...
 */
{
  ...
  if (!fs.existsSync(p)) {
     ...
     *
     * El ancla es `do script`, que es lo que DISPARA un macro, y no la cadena "Keyboard
     * Maestro Engine", que era lo que usaba hasta el 2026-09-19. Nombrar a KM no es
     * dispararle nada: en cuanto se agrego `ventanaAlFrente` —que le pide un token de
     * lectura, arriba en el archivo— la primera mencion quedo antes del activate y esta
     * guarda FALLO SOBRE CODIGO CORRECTO. Que es el peor modo de fallo de una guarda y el
     * que este repo ya pago con `[^)]*?`, con la ventana de 900 caracteres y con las cinco
     * que matchearon su propia prosa: medir por una cadena que aparece por otros motivos.
     */
    const iAct = src3.lastIndexOf("Adobe UXP Developer Tools\" to activate");
    const iMacro = src3.indexOf("do script");
    /*
     * Y EL ACTIVATE TIENE QUE SER INCONDICIONAL. Uno que viva solo adentro del
     * `if (!corriendo("UXP Developer"))` deja sin foco justamente el caso del bug real —UDT
     * ABIERTO pero atras—, que es el 90% de las veces.
     *
     * Encontrado por mutacion el 2026-09-19, y era un agujero VIEJO: sacando el activate de
     * afuera, la version que solo comparaba posiciones seguia diciendo ok porque `lastIndexOf`
     * caia en el de adentro del if. O sea que la guarda cubria "existe un activate en algun
     * lado" y no "UDT queda al frente siempre". Es la guarda contra el caso imaginado dejando
     * pasar el real, por enesima vez en este repo.
     */
    const cuerpoUDT = (() => {
      const i = src3.indexOf("async function prepararUDT(");
      if (i === -1) return "";
      let d = 0;
      for (let k = src3.indexOf("{", i); k < src3.length; k++) {
        if (src3[k] === "{") d++;
        else if (src3[k] === "}") { d--; if (d === 0) return src3.slice(i, k + 1); }
      }
      return "";
    })();
    const iIf = cuerpoUDT.indexOf("if (!corriendo(");
    let finIf = -1;
    if (iIf !== -1) {
      let d = 0;
      for (let k = cuerpoUDT.indexOf("{", iIf); k < cuerpoUDT.length; k++) {
        if (cuerpoUDT[k] === "{") d++;
        else if (cuerpoUDT[k] === "}") { d--; if (d === 0) { finIf = k; break; } }
      }
    }
    const activaSiempre = finIf !== -1 &&
      /Adobe UXP Developer Tools[^\n]*to activate/.test(cuerpoUDT.slice(finIf));
    ...
    } else if (!activaSiempre) {
      mal("`recargar.js` activa UDT solo cuando estaba CERRADO",
        "el bug real es UDT ABIERTO y atras, que es el caso normal: ahi el macro clickea por " +
        "imagen sobre una ventana tapada y el informe culpa al panel. El activate va afuera del " +
        "`if (!corriendo(...))`, no adentro");
...
/* ---------- recargar.js: un CARTEL de Premiere se puede ver, y no se puede adivinar ---------- */

titulo("recargar.js MIRA si hay un cartel de Premiere, y no lo da por ausente cuando no pudo mirar");

/*
 * Medido el 2026-09-18, cableado el 09-19. Un modal de Premiere es INVISIBLE por el panel: con
 * el dialogo en pantalla `estado` contesta su resumen completo, asi que desde este lado no se
 * nota nada. El 2026-09-11 eso termino con la carpeta de un proyecto todavia abierto borrada.
 *
 * Keyboard Maestro si lo ve: `%WindowName%1%` devuelve el titulo de la ventana del frente de la
 * app del frente, y un dialogo trae su propio nombre ("Project Settings") donde el proyecto
 * trae su ruta.
 *
 * Cuatro propiedades, y la primera es la que importa:
 *
 *   1. NO EXISTE "no hay cartel". Si no se pudo mirar, se dice. Un `esProyecto: true` por
 *      defecto seria el falso negativo de este repo por sexta vez, y el daño concreto es
 *      seguir trabajando sobre un Premiere trabado creyendo que esta sano porque contesta.
 *   2. Se comprueba que KM Engine CORRA antes de pedirle el token: un diagnostico no abre
 *      aplicaciones, y que no este corriendo es por si solo una causa de que nada ande.
 *   3. El hallazgo va en la TRABA, que es el texto que ve toda llamada que rebote despues.
 *      Ponerlo solo en la consola es dejarlo donde nadie lo lee.
 *   4. `porQueNoAndubo` mira KM Engine PRIMERO: sin el no se dispara NINGUN macro, y es lo
 *      mas barato de comprobar.
 */
{
  const p = path.join(raiz, "herramientas/recargar.js");
  if (!fs.existsSync(p)) {
    mal("falta `herramientas/recargar.js`", "es el que detecta el cartel que bloquea el Cmd+Q");
  } else {
    const src4 = fs.readFileSync(p, "utf8");
    /* Por CUERPO de funcion y no por ventana de caracteres: este repo ya tuvo una guarda que
       se paso a la funcion de al lado por cortar por tamaño. */
    const cuerpoDe = (nombre) => {
      const i = src4.indexOf(`function ${nombre}(`);
      if (i === -1) return "";
      let d = 0;
      for (let k = src4.indexOf("{", i); k < src4.length; k++) {
        if (src4[k] === "{") d++;
        else if (src4[k] === "}") { d--; if (d === 0) return src4.slice(i, k + 1); }
      }
      return "";
    };
    /* Sin comentarios NI strings: las cuatro cosas que se buscan se NOMBRAN en la prosa que las
       explica y en los mensajes de error, y este repo ya tuvo cinco guardas que matchearon su
       propia prosa —una de ellas por un literal de string—. */
    const limpiar = (t) => t.replace(/\/\*[\s\S]*?\*\//g, "").replace(/^\s*\/\/.*$/gm, "")
      .replace(/"(?:[^"\\\n]|\\.)*"/g, '""').replace(/'(?:[^'\\\n]|\\.)*'/g, "''")
      .replace(/`(?:[^`\\]|\\.)*`/g, "``");

    const vdp = limpiar(cuerpoDe("ventanaDePremiere"));
    const vaf = limpiar(cuerpoDe("ventanaAlFrente"));
    const pqn = limpiar(cuerpoDe("porQueNoAndubo"));

    /* 1. la rama de "el foco no esta en Premiere" tiene que devolver `miro: false`. Se mira la
          rama, no el archivo: un `miro: false` en otro lado no prueba nada. */
    const iRama = vdp.search(/if\s*\(\s*!\s*v\.app\s*\|\|\s*!\s*\/Premiere\//);
    const rama = iRama === -1 ? "" : vdp.slice(iRama, iRama + 260);
    const noInventa = iRama !== -1 && /miro\s*:\s*false/.test(rama) &&
                      /miro\s*:\s*false/.test(limpiar(cuerpoDe("ventanaDePremiere")).split("const titulo")[0]);
    /* Y que el camino de KM que no contesta tambien diga que no pudo. */
    const noInventaKM = /!v\.ok\)\s*return\s*\{\s*miro\s*:\s*false/.test(vdp);

    /* 2. KM Engine comprobado ANTES del token, por POSICION. */
    const iCorriendo = vaf.search(/corriendo\(\s*""\s*\)|corriendo\(/);
    const iToken = vaf.indexOf("process tokens");
    const iTokenReal = iToken !== -1 ? iToken : vaf.search(/osa\(/);
    const noLanzaKM = iCorriendo !== -1 && iTokenReal !== -1 && iCorriendo < iTokenReal;

    /* 3. la rama del cierre fallido MIRA, y el hallazgo entra en la traba.
     *
     * POR DELIMITADOR BALANCEADO, no por ventana de caracteres. Esto cortaba 3000 caracteres
     * desde `if (!cerro) {` y 700 desde `ponerTraba(`, y el 2026-09-21 documentar mejor esa
     * rama —se le agregaron el tope de 180s y la separacion entre "tardando" y "trabado"—
     * empujo el `ponerTraba(` fuera de los 3000 y la guarda fallo SOBRE CODIGO CORRECTO.
     *
     * Es la tercera vez en este repo que una guarda mide de menos por cortar por tamaño: ya
     * paso con los 900 caracteres de la guarda de secuencia y con los 400 del ojito. Una
     * ventana fija mide el LARGO del bloque, no su contenido, asi que crece el codigo y la
     * guarda se rompe sola. */
    const bloqueDesde = (txt, ancla, abre, cierra) => {
      const i = txt.indexOf(ancla);
      if (i === -1) return "";
      let d = 0;
      for (let k = txt.indexOf(abre, i); k >= 0 && k < txt.length; k++) {
        if (txt[k] === abre) d++;
        else if (txt[k] === cierra) { d--; if (d === 0) return txt.slice(i, k + 1); }
      }
      return "";
    };
    const ramaCierre = bloqueDesde(src4, "if (!cerro) {", "{", "}");
    const miraAlFallar = /ventanaDePremiere\(\)/.test(limpiar(ramaCierre));
    /* y `v.miro` tiene que estar DENTRO de los argumentos de `ponerTraba(...)`, no "cerca":
       lo que se exige es que el hallazgo viaje en la traba, no que este escrito al lado. */
    const argsTraba = bloqueDesde(ramaCierre, "ponerTraba(", "(", ")");
    const enLaTraba = argsTraba !== "" && /v\.miro/.test(limpiar(argsTraba));

    /* 4. KM Engine primero en el diagnostico. */
    const iKM = pqn.search(/corriendo\(/);
    const iRemota = pqn.indexOf("sesionRemotaJump");
    const kmPrimero = iKM !== -1 && iRemota !== -1 && iKM < iRemota;

    if (!noInventa || !noInventaKM) {
      mal("`ventanaDePremiere` da por AUSENTE un cartel que no pudo mirar",
        "es el falso negativo de este repo otra vez, y el mas caro de esta familia: el panel " +
        "sigue latiendo con el modal puesto, asi que 'no hay cartel' hace seguir trabajando " +
        "sobre un Premiere trabado. Las respuestas son tres: es el proyecto, es OTRA ventana, " +
        "o NO SE PUDO MIRAR");
    } else if (!noLanzaKM) {
      mal("`ventanaAlFrente` le pide el token a KM sin comprobar que este corriendo",
        "AppleScript LANZA la app al mandarle un `tell`, y un diagnostico no abre aplicaciones. " +
        "Ademas, que KM Engine no este corriendo es por si solo la causa de que ningun macro ande");
    } else if (!miraAlFallar) {
      mal("la rama del cierre fallido no MIRA si hay un cartel",
        "antes decia 'PUEDE haber un dialogo' y dejaba sin separar las dos causas, que mandan a " +
        "lugares distintos: el cartel se resuelve en pantalla y el macro en Keyboard Maestro. " +
        "Buscar del lado equivocado ya costo tres intentos y dos esperas de 60s");
    } else if (!enLaTraba) {
      mal("el hallazgo no entra en la TRABA, solo en la consola",
        "la traba es el texto que ve TODA llamada que rebote despues, o sea lo unico que alguien " +
        "va a leer mas tarde. Un dato que esta en la consola y no ahi es un dato que no esta");
    } else if (!kmPrimero) {
      mal("`porQueNoAndubo` no mira KEYBOARD MAESTRO ENGINE primero",
        "sin el no se dispara NINGUN macro y es lo mas barato de comprobar. Estaba descartado a " +
        "mano en cada diagnostico y nunca escrito, asi que se volvia a mirar cada vez");
    } else {
      ok("`recargar.js` mira el cartel, no lo inventa, no lanza KM, y lo pone en la traba");
    }
  }
}

/* ---------- recargar.js: esperar el cierre NO es esperar a ciegas ---------- */

titulo("La espera del cierre distingue «esta tardando» de «esta trabado», y solo traba el segundo");

/*
 * Medido el 2026-09-21 cerrando el proyecto de un videoclip —483 medios, secuencia de 227 clips—.
 * La espera era de 60s y Premiere tardo MAS: a los 60 todavia estaba escribiendo el .prproj
 * y a los ~160 ya no habia ningun proceso.
 *
 * El informe equivocado no era lo caro. Lo caro es que esa rama **pone la TRABA**, que rebota
 * TODA llamada del bridge hasta que alguien corra `--destrabar` mirando la pantalla. O sea
 * que un reinicio que salio BIEN dejaba el transporte bloqueado y un mensaje mandando a
 * buscar un cartel que no existia.
 *
 * El dato que separa los dos casos es el mtime del .prproj: si Premiere lo toco recien esta
 * guardando y hay que dejarlo; si hace rato que no lo toca y sigue vivo, algo lo frena.
 */

{
  const p = path.join(raiz, "herramientas/recargar.js");
  if (!fs.existsSync(p)) {
    mal("falta `herramientas/recargar.js`", "es el que cierra Premiere antes de reabrirlo");
  } else {
    const src = fs.readFileSync(p, "utf8");
    const limpiar = (t) => t.replace(/\/\*[\s\S]*?\*\//g, "").replace(/^\s*\/\/.*$/gm, "")
      .replace(/"(?:[^"\\\n]|\\.)*"/g, '""').replace(/'(?:[^'\\\n]|\\.)*'/g, "''")
      .replace(/`(?:[^`\\]|\\.)*`/g, "``");
    const bloque = (txt, ancla, abre, cierra) => {
      const i = txt.indexOf(ancla);
      if (i === -1) return "";
      let d = 0;
      for (let k = txt.indexOf(abre, i); k >= 0 && k < txt.length; k++) {
    ...
    const src_ = limpiar(src);

    /* 1. el tope, y que el bucle lo USE en vez de un literal. */
    const mTope = src_.match(/TOPE_CIERRE_S\s*=\s*(\d+)/);
    const tope = mTope ? Number(mTope[1]) : 0;
    const bucleUsaTope = /for\s*\(\s*let\s+i\s*=\s*0\s*;\s*i\s*<\s*TOPE_CIERRE_S\s*;/.test(src_);

    /* 2. dentro de la rama del cierre fallido: se LEE el mtime, y el flag NO trata el
          "no pude leer" como si estuviera guardando. Ese null es el lado por el que este
          repo ya pago un cuadro negro y un clip dado por activo. */
    const rama = limpiar(bloque(src, "if (!cerro) {", "{", "}"));
    const leeMtime = /haceCuantoCambio\(\s*ruta\s*\)/.test(rama);
    const mFlag = rama.match(/seguiaGuardando\s*=\s*([^;]+);/);
    const noNull = mFlag ? /!==\s*null/.test(mFlag[1]) : false;

    /* 3. LA PROPIEDAD QUE IMPORTA: mientras siga guardando, `ponerTraba` NO se alcanza.
          Se mira por BLOQUE BALANCEADO —la rama entera— y no por ventana de caracteres:
          esta misma tanda rompio una guarda que cortaba 3000 caracteres. */
    const ramaGuardando = bloque(rama, "if (seguiaGuardando) {", "{", "}");
    const cortaAntes = /return\s+false/.test(ramaGuardando);
    const sinTrabaAhi = ramaGuardando !== "" && !ramaGuardando.includes("ponerTraba(");
    const iRamaG = rama.indexOf("if (seguiaGuardando) {");
    const iTraba = rama.indexOf("ponerTraba(");
    const trabaDespues = iRamaG !== -1 && iTraba !== -1 && iRamaG < iTraba;

    if (tope < 180 || !bucleUsaTope) {
      mal("la espera del cierre volvio a ser corta o a estar cableada",
        "60s alcanzaban para un proyecto liviano y NO para uno de 483 medios, medido el\n         " +
        "2026-09-21. El tope es " + (tope || "(no se encontro TOPE_CIERRE_S)") +
        " y el bucle " + (bucleUsaTope ? "lo usa" : "NO lo usa"));
    } else if (!leeMtime) {
      mal("la rama del cierre fallido no mira si Premiere sigue GUARDANDO",
        "sin ese dato «esta tardando» y «esta trabado» se ven iguales, y la version vieja\n         " +
        "los daba los dos por «el macro no anduvo»");
    } else if (!noNull) {
      mal("`seguiaGuardando` trata «no pude leer el .prproj» como si estuviera guardando",
        "ese null tiene que ser su propio caso: si no se puede mirar, NO se puede afirmar que\n         " +
        "esta trabajando, y la traba deja de ponerse nunca. Es el falso negativo de este repo");
    } else if (!cortaAntes || !sinTrabaAhi || !trabaDespues) {
      mal("se traba el transporte aunque Premiere siga guardando",
        "es el daño real del bug: un reinicio SANO dejaba el bridge bloqueado hasta que\n         " +
        "alguien corriera `--destrabar`. Mientras el .prproj siga cambiando hay que salir\n         " +
        "ANTES de `ponerTraba`");
    } else {
      ok("la espera del cierre mira el .prproj, distingue tardar de trabarse, y solo traba lo segundo");
    }
  }
}
 ...
 */
{
  ...
  const cuerpo = cuerpoDeFuncion(src, "async function exportar") || "";
 ...
 */
{
   ...
   * pantalla dan `!==`. Medido el 2026-09-05 con el corte real de un videoclip, cuyo material vive bajo
   * `…/Música/<proyecto>`: la guarda imprimio "quiero X · ya esta X" con las dos lineas iguales y
 ...
 */
{
  const VISION = {
    ...
    // las del armado del evento; lo que importa de ellas se CORRE en el bloque siguiente
    "camara.py":        { veredicto: null, nota: "tiron de camara con RANSAC, muestreo corregido" },
    "foco_tramo.py":    { veredicto: null, nota: "nitidez relativa adentro de un tramo" },
    "foco_de_cerca.py": { veredicto: null, nota: "el cuadro mas y menos nitido de cada tramo" },
    "tira_clip.py":     { veredicto: null, nota: "tira de un clip, para in-points y rotacion" },
    "video_comun.py":   { veredicto: null, nota: "geometria y decision de rotacion compartidas" },
  };
...
titulo("Las herramientas de material del evento: controles que se CORREN, no que se leen");

/*
 * `camara.py` y `foco_tramo.py` —con `video_comun.py` de base— se chequean CORRIENDOLAS, porque
 * las tres propiedades que importan no se ven en el texto:
 *
 * 1. Que `camara.py` NO invente tiron sobre un paneo parejo, Y que SIGA viendo un golpe. El
 *    original pedia `fps=25` sobre 59,94, que toma cuadros de fuente de a 2 y de a 3: un paneo
 *    sin un solo golpe de 300 px/s daba 3,9 de tiron, y 9,1 sobre una fuente de 29,97. `camara.py
 *    control` corre el control sobre un video sintetico. Hacen falta las DOS mitades: una
 *    herramienta que devuelve ceros pasa la primera.
 * 2. Que las dos que miden REBOTEN si un clip trae flag de rotacion y nadie decidio, y que CON la
 *    opcion anden. En el evento el flag estaba bien y en un videoclip mal: el default de `familias.py` y
 *    `grilla_angulos.js` habria medido todo el evento acostado. El rebote se reconoce por su
 *    MENSAJE, no por el codigo de salida: un crash tambien sale con 1.
 * 3. Que no pisen un registro ajeno. El original escribia `camara.json` al lado de la lista, que
 *    es el nombre del archivo del proyecto.
 *
 * Necesitan python3 con numpy y OpenCV. Si no estan, esto FALLA diciendo que no pudo: una guarda
 * que informa ok cuando no pudo comprobar no es una guarda.
 */
{
  const py = fs.existsSync("/usr/bin/python3") ? "/usr/bin/python3" : "python3";
  const H = path.join(raiz, "herramientas");
  let cv = null;
  try { execFileSync(py, ["-c", "import numpy, cv2, PIL"], { stdio: "pipe" }); cv = true; }
  catch (e) { cv = String(e.stderr || e.message).trim().split("\n").pop(); }
  if (cv !== true) {
    mal("no se pudieron correr los controles de `camara.py` y `foco_tramo.py`", "falta numpy, OpenCV o Pillow en " + py + ": " + cv);
  } else {
    const corre = (args) => {
      try { execFileSync(py, args, { stdio: "pipe", timeout: 180000 }); return { st: 0, err: "" }; }
      catch (e) { return { st: e.status == null ? -1 : e.status, err: String(e.stderr || e.message) }; }
    };
    const c = corre([path.join(H, "camara.py"), "control"]);
    if (c.st !== 0) mal("`camara.py control` no pasa: el instrumento inventa tiron o no ve el golpe",
      c.err.trim().split("\n").pop());
    else ok("`camara.py` no inventa tiron en un paneo parejo (CFR y VFR) y ve un golpe de 8 px");

    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "material-test-"));
    try {
      const plano = path.join(tmp, "plano.mp4"), rot = path.join(tmp, "rot90.mp4");
      execFileSync("ffmpeg", ["-v", "error", "-f", "lavfi", "-i", "testsrc2=s=320x180:r=25:d=1.2",
        "-pix_fmt", "yuv420p", plano, "-y"], { stdio: "pipe" });
      // el flag se escribe sin recodificar: recodificando, ffmpeg APLICA la rotacion y no queda flag
      execFileSync("ffmpeg", ["-v", "error", "-display_rotation", "90", "-i", plano, "-c", "copy", rot, "-y"], { stdio: "pipe" });
      const REBOTE = /ROTACION SIN DECIDIR/;
      const problemas = [];
      for (const [nom, args] of [["camara.py", ["medir", rot]], ["foco_tramo.py", [rot]]]) {
        const sal = path.join(tmp, nom + ".json");
        const sin = corre([path.join(H, nom), ...args, "--salida", sal]);
        if (sin.st === 0 || !REBOTE.test(sin.err) || fs.existsSync(sal)) {
          problemas.push(`\`${nom}\` sin --rotacion sobre un clip con flag: ` +
            (sin.st === 0 ? "MIDIO" : (REBOTE.test(sin.err) ? "rebota pero escribio la salida" : "fallo por OTRA cosa: " + sin.err.trim().split("\n").pop())));
          continue;
        }
        const con = corre([path.join(H, nom), ...args, "--salida", sal, "--rotacion", "respetar"]);
        if (con.st !== 0 || !fs.existsSync(sal)) {
          problemas.push(`\`${nom}\` CON --rotacion no mide: ${con.err.trim().split("\n").pop()}`);
        }
      }
      const ajeno = path.join(tmp, "camara.json");
      const antes = JSON.stringify([{ clip: "rot90.mp4", ruta: rot, tiron: [1] }]);
      fs.writeFileSync(ajeno, antes);
      const pisa = corre([path.join(H, "camara.py"), "medir", rot, "--rotacion", "respetar", "--salida", ajeno]);
      if (pisa.st === 0 || fs.readFileSync(ajeno, "utf8") !== antes) {
        problemas.push("`camara.py` pisa un camara.json con mediciones de otro metodo");
      }
      if (problemas.length) mal("las herramientas de material no se portan como deben", problemas.join("\n         "));
      else ok("rebotan un flag de rotacion sin decidir, miden con la opcion y no pisan un registro ajeno");
    } catch (e) {
      mal("no se pudo armar el clip de prueba con flag de rotacion", String(e.stderr || e.message).trim().slice(0, 200));
    } finally {
 ...
 */
{
  ...
  if (!fs.existsSync(pc)) mal("falta `herramientas/plancha_corte.js`");
    ...
    // Las del evento toman la opcion de `video_comun.py`, que es donde vive el -noautorotate; lo
    // que valida la decision se CORRE en el bloque "Las herramientas de material del evento".
    const DE_CAMARA = ["broll.js", "grilla_angulos.js", "familias.py", "video_comun.py"];
 ...
 */
{
  ...
  /* Ancla en la PROPIEDAD —que la salida se derive de `entQuiero`— y no en como se escribe.
   * La version anterior exigia el literal `Number((entQuiero + p.dura)` y por eso fallo sobre
   * codigo correcto el 2026-09-21, cuando se saco el `.toFixed(3)` que sacaba el valor de la
   * grilla fuera de 25fps. Una guarda que se rompe al arreglar un bug al lado mide la forma. */
  const salidaSobreEse = /salida:[^;]*?entQuiero\s*\+\s*p\.dura/.test(codigo);
  if (!acepta) {
...
{
  /* Se DERIVA del manifest en vez de clavarse. Con la ruta fija, cambiar el `id` dejaba
     este chequeo mirando una carpeta que no existe —o sea PASANDO SIEMPRE, que es peor
     que no tenerlo: una guarda que no puede fallar no protege nada. */
 ...
 * fue mutear A2/A3/A4 a mano en CORTE. Verificado contra Premiere ese dia, antes y
 * despues del arreglo.
 *
...
/* ---------- una lectura fallada NO es una pista vacia ---------- */

/*
 * `colocar_propuesta.js` leia la pista para limpiarla dentro de un `try/catch` que
 * convertia el fallo en lista vacia. Informaba "0 clip(s)", no borraba nada, y despues
 * COLOCABA encima de una pista que no se pudo inspeccionar. Visto en vivo con la pista
 * teniendo 88.
 *
 * Y el `--limpiar` contaba los `borrar` que NO TIRARON como exito, sin releer. El
 * veredicto sale del estado, no de que la llamada no haya tirado.
 */

titulo("Limpiar una pista: la lectura aborta si falla, y el barrido se verifica releyendo");

{
  const f = path.join(raiz, "herramientas/colocar_propuesta.js");
  const src = fs.readFileSync(f, "utf8");
  const cuerpo = src.replace(/\/\*[\s\S]*?\*\//g, "").replace(/\/\/[^\n]*/g, "");

  const tragando = /catch\s*\([^)]*\)\s*\{\s*cs\s*=\s*\[\]/.test(cuerpo);
  const mLector = cuerpo.match(/async function clipsDe\([\s\S]*?\n\}/);
  const limpiar = cuerpo.indexOf("if (LIMPIAR) {");
  const borrarEnLimpiar = cuerpo.indexOf('enviar("borrar"', limpiar < 0 ? 0 : limpiar);
  const releeDespues = cuerpo.indexOf("clipsDe(", borrarEnLimpiar < 0 ? 0 : borrarEnLimpiar);

  if (tragando) {
    mal("`colocar_propuesta.js` vuelve a convertir un fallo de lectura en lista vacia",
      "informa '0 clip(s)' y despues coloca encima de una pista que no pudo inspeccionar");
  } else if (!mLector) {
    mal("no existe `clipsDe` en colocar_propuesta.js",
      "la lectura de la pista a limpiar tiene que estar en un solo lugar que aborte si falla");
  } else if (!/process\.exit\(1\)/.test(mLector[0])) {
    mal("`clipsDe` no aborta cuando la lectura falla",
      "devolver una lista vacia ahi es el contador ciego: una lectura fallada no es una pista vacia");
  } else if (!/enviar\("clips", g\(/.test(mLector[0])) {
    mal("`clipsDe` no pasa la guarda de secuencia",
      "el `borrar` que le sigue si la pasa: leer de una secuencia y borrar en otra es medio desastre");
  } else if (limpiar < 0 || borrarEnLimpiar < 0 || releeDespues < 0) {
    mal("el bloque `--limpiar` no relee las pistas despues de barrerlas",
      "contar los `borrar` que no tiraron no dice que la pista quedo vacia");
  } else {
    ok("la lectura aborta si falla y `--limpiar` verifica releyendo");
  }
}

/* ---------- un proyecto de RECUPERACION no pasa la guarda ---------- */

/*
 * La guarda `proyecto` compara por SUBCADENA a proposito —pedir "X" y que enganche
 * "X - nombre largo" se usa— pero Premiere nombra sus recuperaciones como el proyecto MAS
 * un GUID, asi que la CONTIENEN y pasaban. Medido: una tanda entera se ejecuto contra una
 * recuperacion con la guarda contestando que si, y todo quedo escrito en la carpeta de
 * auto-save.
 *
 * Se chequea POR POSICION: el rechazo tiene que estar ANTES de la comparacion por nombre,
 * porque estar parado en una recuperacion es un problema tenga el nombre que tenga.
 */

titulo("La guarda `proyecto` rechaza un auto-save o una recuperacion");

{
  const g = srcComandos.indexOf('if (typeof p.proyecto === "string"');
  const cuerpo = g < 0 ? "" : srcComandos.slice(g, g + 4000).replace(/\/\*[\s\S]*?\*\//g, "");
  const auto = cuerpo.search(/Auto-Save/);
  const porNombre = cuerpo.search(/indexOf\(p\.proyecto/);
  if (g < 0) {
    mal("no se encontro la guarda `proyecto` en comandos.js");
  } else if (auto < 0) {
    mal("la guarda `proyecto` no rechaza un auto-save / recuperacion",
      "Premiere nombra la recuperacion como el proyecto MAS un GUID, asi que CONTIENE el\n         " +
      "nombre pedido y la comparacion por subcadena la deja pasar. Todo se guarda en\n         " +
      "`Adobe Premiere Pro Auto-Save/` y el .prproj bueno queda igual que estaba.");
  } else if (porNombre >= 0 && auto > porNombre) {
    mal("el rechazo del auto-save esta DESPUES de la comparacion por nombre",
      "una recuperacion cuyo nombre coincide pasa antes de llegar al rechazo");
  } else if (!/test\(ruta\)/.test(cuerpo)) {
    mal("el rechazo del auto-save no mira la RUTA",
      "decidirlo por el nombre depende del formato del GUID; la carpeta es la que Premiere usa");
  } else {
    ok("la guarda `proyecto` rechaza auto-save y recuperaciones, por RUTA y antes del nombre");
  }
}

/* ---------- cotejar dos transcripciones: sin acentos, y sin inventar una tercera ---------- */

/*
 * `cotejar_transcripcion.js` cruza Whisper contra la transcripcion nativa de Premiere y
 * devuelve SOLO donde discrepan. Dos propiedades lo sostienen y las dos rompen en
 * silencio:
 *
 *   1. La comparacion IGNORA ACENTOS. Sin eso, "mirame" contra "mírame" cuenta como
 *      discrepancia y el informe se llena de ruido que tapa las de verdad — medido: en un
 *      clip de 18s hay una sola diferencia de tilde y cuatro discrepancias reales.
 *   2. NO produce una transcripcion "mejor". Elegir por su cuenta cual de las dos tiene
 *      razon seria inventar una tercera fuente sin haber escuchado nada, que es
 *      exactamente lo que la herramienta existe para evitar.
 */

titulo("Cotejar transcripciones: ignora acentos y no inventa una tercera version");

{
  const f = path.join(raiz, "herramientas/cotejar_transcripcion.js");
  if (!fs.existsSync(f)) {
    mal("falta herramientas/cotejar_transcripcion.js");
  } else {
    const src = fs.readFileSync(f, "utf8");
    const cuerpo = src.replace(/\/\*[\s\S]*?\*\//g, "").replace(/\/\/[^\n]*/g, "");
    const mNorm = cuerpo.match(/const norm = [\s\S]*?;/);
    const sinAcentos = mNorm && /normalize\("NFD"\)/.test(mNorm[0]) && /replace\(/.test(mNorm[0]);
    const inventa = /\bmejor(Texto|Version)\b|corregid[oa]|textoFinal/.test(cuerpo);
    if (!mNorm) {
      mal("no se encontro la normalizacion de `cotejar_transcripcion.js`");
    } else if (!sinAcentos) {
      mal("la comparacion NO ignora acentos",
        "cada tilde distinta cuenta como discrepancia y el informe se llena de ruido que\n         " +
        "tapa las diferencias reales");
    } else if (inventa) {
      mal("la herramienta parece elegir una version 'correcta'",
        "su trabajo es decir DONDE discrepan, no cual tiene razon: eso se decide escuchando");
    } else {
      ok("ignora acentos y solo informa discrepancias");
    }
  }
}

/* ---------- el prevuelo de rutas vive en el TRANSPORTE, no en el despachador ---------- */
...
/* ---------- fusionar transcripciones: las costuras quedan a la vista ---------- */

/*
 * `fusionar_transcripcion.js` combina Whisper con la transcripcion nativa. Tres cosas lo
 * sostienen y las tres rompen en silencio:
 *
 *   1. Las palabras traidas de Premiere tienen que viajar con `tiempoExacto: false`.
 *      Premiere da tiempos POR SEGMENTO y este repo CORTA sobre tiempos de palabra: una
 *      palabra con el tiempo de su segmento es un corte mal puesto esperando, y nada lo
 *      avisaria despues.
 *   2. Un conflicto tiene que guardar la ALTERNATIVA. Quedarse con una y tirar la otra es
 *      elegir sin haber escuchado, que es lo que estas dos herramientas existen para
 *      evitar.
 *   3. La salida tiene que decir que es un BORRADOR. Un JSON que se ve igual que una
 *      transcripcion verificada se usa como si lo fuera.
 */

titulo("Fusionar transcripciones: marca el origen, el tiempo y que es un borrador");

{
  const f = path.join(raiz, "herramientas/fusionar_transcripcion.js");
  if (!fs.existsSync(f)) {
    mal("falta herramientas/fusionar_transcripcion.js");
  } else {
    /* Sin comentarios Y SIN STRINGS. La primera version solo sacaba comentarios y
       matcheaba el texto de la `advertencia`, que menciona `tiempoExacto: false` como
       prosa: la mutacion que ponia `true` en las palabras de Premiere pasaba igual. Es la
       guarda ciega de CLAUDE.md con un disfraz nuevo — no un comentario, un LITERAL. */
    const cuerpo = fs.readFileSync(f, "utf8")
      .replace(/\/\*[\s\S]*?\*\//g, "").replace(/\/\/[^\n]*/g, "")
      .replace(/"(?:[^"\\]|\\.)*"/g, '""').replace(/'(?:[^'\\]|\\.)*'/g, "''");
    /* Y se mira DENTRO de cada funcion, no en todo el archivo: lo que importa es que las
       palabras de Premiere salgan con el tiempo marcado como aproximado. */
    const fnP = (cuerpo.match(/function palabrasPremiere\([\s\S]*?\n\}/) || [""])[0];
    const fnW = (cuerpo.match(/function palabrasWhisper\([\s\S]*?\n\}/) || [""])[0];
    const marcaTiempo = /tiempoExacto:\s*false/.test(fnP);
    const baseWhisper = /tiempoExacto:\s*true/.test(fnW);
    const alternativa = /alternativa:/.test(cuerpo);
    const crudo = fs.readFileSync(f, "utf8");
    const avisa = /advertencia:/.test(crudo) && /BORRADOR/.test(crudo);
    if (!marcaTiempo || !baseWhisper) {
      mal("la fusion no distingue el tiempo EXACTO del aproximado",
        "las palabras de Premiere llevan el tiempo de su SEGMENTO; cortar sobre ellas pone el\n         " +
        "corte donde no va, y nada lo avisa despues");
    } else if (!alternativa) {
      mal("un conflicto no guarda la alternativa",
        "quedarse con una version y tirar la otra es elegir sin escuchar");
    } else if (!avisa) {
      mal("la salida no dice que es un BORRADOR",
        "un JSON que se ve igual que una transcripcion verificada se usa como si lo fuera");
    } else {
      ok("marca origen y exactitud del tiempo, guarda la alternativa y avisa que es borrador");
    }
  }
}

/* ---------- `sinDialogos` no puede ser el default, y tiene que decirlo ---------- */

/*
 * `abrirProyecto` puede apagar el modal de medios faltantes —el que BLOQUEA el Cmd+Q y por
 * lo tanto rompe `recargar.js --reiniciar`— pero eso NO puede ser el comportamiento por
 * defecto: suprimir un aviso de medios faltantes tambien lo esconde de alguien que
 * necesitaba verlo.
 *
 * Y si se apago, el RESUMEN tiene que decirlo. Un proyecto que abrio callando que le
 * faltan medios, sin dejar rastro en lo unico que se lee, es peor que el modal.
 */

titulo("Abrir sin diálogos: se pide, no es el default, y queda dicho en el resumen");

{
  const m = srcComandos.match(/async function abrirProyecto\([\s\S]*?\n\}/);
  if (!m) {
    mal("no se encontro `abrirProyecto`");
  } else {
    const cuerpo = m[0].replace(/\/\*[\s\S]*?\*\//g, "").replace(/\/\/[^\n]*/g, "");
    /* POR POSICION, no por presencia: `params.sinDialogos === true` aparece TAMBIEN en la
       linea del resumen, asi que buscarlo en todo el cuerpo matchea aunque la condicion
       que gatea la construccion se haya sacado. Verificado: la mutacion que ponia
       `if (true)` pasaba igual. Quinta vez que una guarda matchea otro uso del mismo
       token en este repo. */
    const iOpts = cuerpo.indexOf("new ppro.OpenProjectOptions()");
    const ventana = iOpts < 0 ? "" : cuerpo.slice(Math.max(0, iOpts - 220), iOpts);
    const condicionado = /params\.sinDialogos\s*===\s*true/.test(ventana);
    const construye = iOpts >= 0;
    const apaga = /setShowLocateFileDialog/.test(cuerpo);
    const enResumen = /sinDialogos === true \?[^\n]*di[aá]logos/.test(cuerpo) ||
                      /di[aá]logos:\s*\$\{estadoOpciones\}/.test(cuerpo);
    const respaldo = /open\(ruta\) SIN opciones|\["open\(ruta\)"/.test(cuerpo);
    if (!construye || !apaga) {
      mal("`abrirProyecto` no puede apagar el diálogo de medios faltantes",
        "es el modal que bloquea el Cmd+Q y cuelga el reinicio automatico 60s");
    } else if (!condicionado) {
      mal("los diálogos se apagan SIEMPRE, no sólo cuando se piden",
        "suprimir el aviso de medios faltantes por defecto lo esconde de quien necesitaba verlo");
    } else if (!enResumen) {
      mal("no dice en el RESUMEN que apagó los diálogos",
        "un proyecto que abrio callando que le faltan medios, sin rastro en lo unico que se lee,\n         " +
        "es peor que el modal");
    } else if (!respaldo) {
      mal("no queda una forma sin opciones como respaldo",
        "si `OpenProjectOptions` cambia o falla, tiene que poder abrir igual");
    } else {
      ok("se pide con `sinDialogos`, no es el default, y el resumen lo informa");
    }
  }
}
...
titulo("`transicion` conserva lo que costo medirla");

{
  ...
  const cT = cuerpoDe("transicion");

  if (!cT) {
    mal("no existe el verbo `transicion`");
  } else {
    /*
     * 1. El DEFAULT es el Cross Dissolve MODERNO. `ADBE Cross Dissolve` entra como
     *    "Cross Dissolve (Legacy)" —lo cazo el editor mirando el timeline, no una
     *    verificacion mia— y el matchName del bueno salio del XML del proyecto de prueba.
     */
    const defModerno = /params\.matchName\s*:\s*"AE\.AE_Impact_Dissolve"/.test(cT);
    /*
     * 2. Que NO toca el audio va en el RESUMEN, no solo en el comentario: `TransitionFactory`
     *    solo expone `createVideoTransition` y el clip de audio no tiene ningun metodo de
     *    transicion —medido reflejando los dos—. Un fundido de video sin crossfade es medio
     *    trabajo, y un dato que no llega al resumen es un dato que no esta.
     */
    const avisaAudio = /resumen[\s\S]{0,1200}?AUDIO NO se toca/.test(cT);
    /*
     * 3. TODO adentro de UN `lockedAccess` y sin un solo `await`: creado afuera, el objeto de
     *    transicion se invalida y tira "The script object is no longer valid".
     */
    const lock = cT.match(/lockedAccess\(\(\)\s*=>\s*\{[\s\S]*?\n\s{4}\}\);/);
    const lockLimpio = lock ? !/\bawait\b/.test(lock[0]) : false;

    if (!defModerno) {
      mal("`transicion` ya no tiene de default el Cross Dissolve moderno",
        "`ADBE Cross Dissolve` entra como «Cross Dissolve (Legacy)», que NO es el que Premiere\n         " +
        "pone con Cmd+D. El bueno es `AE.AE_Impact_Dissolve`, leido del XML del proyecto de prueba");
    } else if (!avisaAudio) {
      mal("`transicion` no avisa en el RESUMEN que el audio queda sin tocar",
        "la API no tiene transiciones de audio, asi que el crossfade se hace a mano: si el verbo\n         " +
        "no lo dice, se entrega un fundido a medias sin que nadie se entere");
    } else if (!lock) {
      mal("`transicion` ya no crea y ejecuta adentro de un `lockedAccess`");
    } else if (!lockLimpio) {
      mal("`transicion` tiene un `await` adentro del `lockedAccess`",
        "el objeto de transicion se invalida entre crearlo y usarlo: tira\n         " +
        '"The script object is no longer valid". Es la misma leccion que costo `createBinAction`');
    } else {
      ok("`transicion` usa el dissolve moderno, avisa que el audio no se toca, y crea y ejecuta dentro del lock");
    }
  }
}

titulo("`cortar` dice CUANTO movio el corte, y no usa variables de otro verbo");

{
  ...
  const cCortar = cuerpoDe("cortar");

  if (!cCortar) {
    mal("no existe el verbo `cortar`");
  } else {
    /*
     * El 2026-09-19 esta linea decia `dichos`, que es una variable de `editarMarcador`.
     * En `cortar` no existe, asi que el verbo CORTABA, reponia el Motion y limpiaba los
     * in/out, y recien entonces tiraba ReferenceError armando el mensaje. Quien lo
     * llamaba veia un error y concluia que no habia pasado nada: un FALSO NEGATIVO sobre
     * un corte que SI ocurrio, que es lo que hace que alguien reintente y corte dos veces.
     * Se encontro usandolo, no leyendolo.
     */
    const declara = /\blet\s+cuantizado\b/.test(cCortar);
    const enElResumen = /resumen[\s\S]{0,4000}?\$\{cuantizado\}/.test(cCortar);
    const ajena = /\bdichos\b/.test(cCortar);

    if (!declara) {
      mal("`cortar` ya no declara `cuantizado`",
        "es el dato de CUANTO se movio el corte para caer en cuadro");
    } else if (ajena) {
      mal("`cortar` usa `dichos`, que es de `editarMarcador`",
        "aca no existe: el verbo corta y DESPUES tira ReferenceError armando el resumen,\n         " +
        "asi que informa un fallo sobre un corte que si ocurrio. Si alguien reintenta, corta dos veces");
    } else if (!enElResumen) {
      mal("`cortar` calcula `cuantizado` y no lo pone en el RESUMEN",
        "un dato que esta en la respuesta y no en el resumen es un dato que no esta:\n         " +
        "el corte se movio al cuadro y nadie se entera");
    } else {
      ok("`cortar` declara `cuantizado`, lo pone en el resumen y no toca variables de otro verbo");
    }
  }
}
 ...
 * podia releer lo que acababa de escribir, y los 26 marcadores de seccion de un evento hubo que
...
/*
 * El match por subcadena es deliberado —pedir "Proyecto" y que enganche "Proyecto - Corte final"
 * se usa— asi que sacarlo romperia llamadas correctas. Lo que quedaba abierto es que el mismo
...
/* ---------- la guarda `secuencia` y el parcial AMBIGUO ---------- */

/*
 * El mismo agujero que el de arriba, un nivel mas abajo y con un caso real: un proyecto con las
 * dos versiones de un video tiene "SPOT v3" y "SPOT v3 ENG", y pedir la primera PASABA
 * con la segunda activa. El 2026-09-17 eso estuvo a punto de colocar 30 subtitulos encima de los
 * 30 que ya tenia la otra secuencia; lo freno el colocador al ver la pista ocupada, no la guarda.
 *
 * Se exigen las mismas tres propiedades que a la de `proyecto`, y por las mismas razones.
 */
{
  const c = srcComandos.replace(/\/\*[\s\S]*?\*\//g, "").replace(/\/\/[^\n]*/g, "");
  const iG = c.indexOf('typeof p.secuencia === "string"');
  const trozo = iG === -1 ? "" : c.slice(iG, iG + 4000);
  const desambigua = /coinciden\.length > 1/.test(trozo);
  const soloParcial = /nombre\.toLowerCase\(\) !== p\.secuencia\.toLowerCase\(\)/.test(trozo);
  const noFrenaSinSaber = /catch \([^)]*\) \{ todas = \[\]; \}/.test(trozo);
  if (iG === -1) {
    mal("no se encontró la guarda `secuencia` en el despachador");
  } else if (!desambigua) {
    mal("la guarda `secuencia` acepta un parcial que coincide con varias secuencias",
        "\"SPOT v3\" pasa con \"SPOT v3 ENG\" activa, que es la confusión que la guarda existe para evitar");
  } else if (!soloParcial) {
    mal("la desambiguación de `secuencia` corre también cuando el nombre coincide EXACTO",
        "un nombre completo no tiene nada que preguntar: rechazarlo sería rechazar uso correcto");
  } else if (!noFrenaSinSaber) {
    mal("la guarda `secuencia` frena cuando no pudo enumerar las secuencias",
        "no poder averiguar no es estar en peligro");
  } else {
    ok("la guarda `secuencia` rebota si el parcial es ambiguo, sólo cuando es parcial, y no frena a ciegas");
  }
}


/* ---------- el OJITO del clip: `clips` lo devuelve y NO lo asume ---------- */

/*
 * `desactivar` ESCRIBE el ojito y `clips` --el verbo con el que uno mira una pista-- no lo
 * devolvia. Era write-only, y peor: quien lo leyera de la fila obtenia `undefined`, que es falsy,
 * asi que `filter(x => x.desactivado)` daba CERO sobre una pista entera deshabilitada. Paso el
 * 2026-09-17 y mando a investigar por que no sonaba algo que estaba apagado a la vista.
 *
 * Tres propiedades. La del `null` es la que importa: `false` significa "se ve", asi que tragarse
 * el error devolviendo false informaria como activo un clip que no se pudo mirar.
 */
{
  const c = srcComandos.replace(/\/\*[\s\S]*?\*\//g, "").replace(/\/\/[^\n]*/g, "");
  const iC = c.indexOf("async function clips(");
  const cuerpo = iC === -1 ? "" : c.slice(iC, iC + 6000);
  const devuelve = /desactivado:\s*await ojito\(/.test(cuerpo);
  const enResumen = /off\.length/.test(cuerpo) && /OJITO APAGADO/.test(c.slice(iC, iC + 6000));
  const iO = c.indexOf("async function ojito(");
  /* Acotado al cuerpo de `ojito`, NO a una ventana de N caracteres: la primera versión
     usaba 400 y se pasaba a la función de al lado, que sí tiene `catch { return false }`.
     Es la guarda que mide de más — el mismo achaque, del otro lado, que las que matchean
     su propia prosa. */
  const finO = c.indexOf("\nasync function", iO + 1);
  const trozoO = iO === -1 ? "" : c.slice(iO, finO === -1 ? iO + 400 : finO);
  const noAsume = /return null;/.test(trozoO) && !/catch[^}]*return false/.test(trozoO);
  if (iC === -1 || iO === -1) {
    mal("falta el verbo `clips` o el helper `ojito`");
  } else if (!devuelve) {
    mal("`clips` no devuelve el estado del ojito de cada clip",
        "leer ese campo da undefined, que es falsy: se cuentan 0 desactivados sobre una pista apagada entera");
  } else if (!enResumen) {
    mal("`clips` no avisa en el RESUMEN que hay clips con el ojito apagado",
        "un dato que está en la respuesta y no en el resumen es un dato que no está");
  } else if (!noAsume) {
    mal("`ojito` devuelve false cuando no pudo leer, en vez de null",
        "false significa «se ve»: informaría como activo un clip que no se pudo mirar");
  } else {
    ok("`clips` devuelve el ojito, lo avisa en el resumen, y no lo asume `false` al fallar");
  }
}
  ...
  if (!m) {
    ...
    if (!fn) {
      ...
      const casos = [
        ...
        ["item normal", { name: "FX3_3602.MP4" }, "FX3_3602.MP4"],
...
/* ---------- la sesion remota: el detector viejo daba FALSO NEGATIVO ---------- */

/*
 * Con una sesion remota abierta KM no inyecta eventos — es la primera causa anotada de "el macro
 * no anduvo". Lo que no servia era COMO se detectaba: contar conexiones ESTABLISHED por PID dio
 * **0 con el editor conectado**, porque `lsof -nPi -a -p <pid>` devuelve cero lineas para esos
 * procesos. Se creyo dos veces que ese 0 significaba algo.
 *
 * Lo medido: desconectado hay 2 JumpConnect con ppid 1; conectado aparecen dos HIJOS con
 * ppid = un JumpConnect. La marca es "proceso cuyo padre es OTRA instancia del mismo binario", y
 * NO "tiene hijos": Chrome Remote Desktop tiene un hijo permanente, pero con otro nombre.
 *
 * El chequeo EJECUTA la funcion con las dos salidas de `ps`, porque un detector probado en una
 * sola direccion no esta probado — y este ya se equivoco por eso.
 */
{
  const src = fs.readFileSync(path.join(__dirname, "herramientas", "recargar.js"), "utf8");
  const m = src.match(/function sesionRemotaJump\(salidaPs\)[\s\S]*?\n\}/);
  const c = src.replace(/\/\*[\s\S]*?\*\//g, "").replace(/\/\/[^\n]*/g, "");
  if (!m) {
    mal("`recargar.js` no detecta la sesión remota", "es la primera causa de que el macro no cierre Premiere");
  } else if (!/sesionRemotaJump\(\)/.test(c.slice(c.indexOf("function porQueNoAndubo")))) {
    mal("el diagnóstico no nombra la sesión remota");
  } else {
    let fn = null;
    try { fn = eval("(" + m[0].replace(/^function sesionRemotaJump/, "function") + ")"); }
    catch (e) { fn = null; }
    const DESCONECTADO =
      "  276     1 /Applications/Jump Desktop Connect.app/Contents/MacOS/JumpConnect\n" +
      "24015     1 /Applications/Jump Desktop Connect.app/Contents/MacOS/JumpConnect\n" +
      "  673     1 /Library/PrivilegedHelperTools/ChromeRemoteDesktopHost.app/Contents/MacOS/remoting_me2me_host_service\n" +
      "  804   673 /Library/PrivilegedHelperTools/ChromeRemoteDesktopHost.app/Contents/MacOS/remoting_me2me_host\n";
    const CONECTADO = DESCONECTADO +
      "72420   276 /Applications/Jump Desktop Connect.app/Contents/MacOS/JumpConnect\n" +
      "72421 24015 /Applications/Jump Desktop Connect.app/Contents/MacOS/JumpConnect\n";
    if (!fn) {
      mal("`sesionRemotaJump` no se pudo evaluar", "el chequeo la ejecuta, no la lee");
    } else if (fn(CONECTADO) !== true) {
      mal("`sesionRemotaJump` NO ve una sesión abierta", "es el falso negativo que ya se creyó dos veces");
    } else if (fn(DESCONECTADO) !== false) {
      mal("`sesionRemotaJump` ve una sesión donde no la hay",
          "Chrome Remote Desktop tiene un hijo permanente: la marca es el hijo del MISMO binario");
    } else {
...
console.log("");
const MOTORES_ESPERADOS = ["premiere", "scribe", "whisper"];

titulo("audio.js: la key del entorno, el tiempo de FUENTE, y un umbral que no se transfiere");

/*
 * Seis propiedades de `herramientas/audio.js`. Las seis salen de algo que se
 * rompio el 2026-09-21 al escribirla, y las seis rompen EN SILENCIO: la
 * herramienta sigue contestando, con un dato peor.
 */
{
  const ruta = path.join(raiz, "herramientas/audio.js");
  const src = fs.readFileSync(ruta, "utf8");
  // Sin comentarios: este repo ya tuvo cinco guardas que matchearon su propia prosa.
  const limpio = src.replace(/\/\*[\s\S]*?\*\//g, "").replace(/^\s*\/\/.*$/gm, "");

  /* Devuelve el cuerpo de una funcion por LLAVES BALANCEADAS, no por tamano de
   * ventana: cortar N caracteres ya hizo fallar dos guardas de este archivo, una
   * por medir de mas y otra por medir de menos. */
  function cuerpoDe(txt, firma) {
    ...
    for (let k = j; k < txt.length; k++) {
      if (txt[k] === "{") n++;
      else if (txt[k] === "}" && --n === 0) return txt.slice(j, k + 1);
    }
    return null;
  }

  const scribe = cuerpoDe(limpio, "async function palabrasScribe");
  const prem = cuerpoDe(limpio, "async function palabrasPremiere");
  const main = cuerpoDe(limpio, "async function main");

  // 1 — la key SOLO del entorno. Por argumento queda visible en `ps`.
  if (!scribe) mal("audio.js: no encuentro `palabrasScribe`");
  else if (!/process\.env\.ELEVENLABS_API_KEY/.test(scribe))
    mal("audio.js: `palabrasScribe` no lee la key de $ELEVENLABS_API_KEY");
  else if (/cfg\.(key|apiKey|token)|--key/.test(limpio))
    mal("audio.js: la key se puede pasar por argumento", "queda visible en `ps` mientras dura el pedido");
  else ok("audio.js: la key de ElevenLabs sale solo del entorno");

  // 2 — tiempo de FUENTE. `desde`/`hasta` son de TIMELINE si el medio esta montado.
  if (!prem) mal("audio.js: no encuentro `palabrasPremiere`");
  else if (!/desdeFuente/.test(prem) || !/hastaFuente/.test(prem))
    mal("audio.js: `palabrasPremiere` no usa desdeFuente/hastaFuente",
        "esta herramienta promete segundos de la FUENTE; con el medio montado, `desde` es del TIMELINE y todo lo que corte contra eso cae en otro lado");
  else ok("audio.js: los tiempos de Premiere salen del tiempo de FUENTE");

  // 3 — dura y confianza no se tiran. Premiere las da y la primera version las perdia.
  if (prem && (!/w\.confianza/.test(prem) || !/w\.finDeOracion/.test(prem)))
    mal("audio.js: `palabrasPremiere` descarta la confianza o el finDeOracion que Premiere SI devuelve");
  else if (prem) ok("audio.js: se conservan la confianza y el finDeOracion nativos");

  // 4 — el umbral esta medido sobre premiere. Scribe devuelve logprob: marcaria el 100%.
  if (!main) mal("audio.js: no encuentro `main`");
  else {
    const i = main.indexOf("UMBRAL_CONFIANZA");
    const antes = i === -1 ? "" : main.slice(Math.max(0, i - 260), i);
    if (i === -1) mal("audio.js: `main` no usa UMBRAL_CONFIANZA");
    else if (!/motor\s*===\s*"premiere"/.test(antes))
      mal("audio.js: el umbral de confianza se aplica a CUALQUIER motor",
          "esta medido solo contra Premiere; Scribe devuelve logprob y el umbral marcaria el 100% de las palabras");
    else ok("audio.js: el umbral de confianza se aplica solo al motor donde se midio");
  }

  // 5 — los modelos de whisper NO se exigen a los otros motores: rechazar uso correcto es el peor fallo.
  if (main) {
    const i = main.indexOf("cfg.modelo");
    const antes = i === -1 ? "" : main.slice(Math.max(0, i - 300), i);
    if (i === -1) mal("audio.js: `main` no chequea el modelo de whisper");
    else if (!/motor\s*===\s*"whisper"/.test(antes))
      mal("audio.js: el chequeo de los modelos de whisper corre para todos los motores",
          "haria rebotar a `--motor premiere`, que no los usa");
    else ok("audio.js: los modelos de whisper se exigen solo en la rama de whisper");
  }

  // 6 — el achaque del motor va en el RESUMEN, no solo en el JSON.
  if (!/ACHAQUES\s*=\s*{/.test(limpio)) mal("audio.js: no existe la tabla ACHAQUES");
  else if (!MOTORES_ESPERADOS.every((m) => new RegExp("\\b" + m + ":").test(limpio.slice(limpio.indexOf("ACHAQUES")))))
    mal("audio.js: ACHAQUES no cubre los tres motores");
  else if (!main || !/ACHAQUES\[cfg\.motor\]/.test(main))
    mal("audio.js: el resumen no nombra el achaque del motor",
        "un dato que esta en la respuesta y no en el resumen es un dato que no esta");
  else ok("audio.js: el resumen dice que NO puede el motor elegido");

  /* 8 — el resumen nombra LOS TRES archivos. Se calculaban y no se imprimian, asi que la
   * herramienta escribia tres y nombraba uno; sin `--destino` escribe al lado del medio y eso
   * dejo dos archivos dentro de una carpeta de crudos sin que nada lo dijera. */
  if (main && (!/\$\{paraPremiere\}/.test(main) || !/\$\{srt\}/.test(main)))
    mal("audio.js: el resumen no nombra el .srt y el .premiere.json que acaba de escribir",
        "sin `--destino` se escriben al lado del medio, y nadie se entera");
  else if (main && !/cfg\.destino \?/.test(main))
    mal("audio.js: el resumen no avisa que sin `--destino` escribe al lado del medio");
  else if (main) ok("audio.js: el resumen nombra los tres archivos y donde quedaron");

  // 7 — `flojas` arranca en null y no en 0: cero diria "esta todo bien" sobre algo que no se midio.
  if (main && /const flojas\s*=\s*0\b/.test(main))
    mal("audio.js: `flojas` arranca en 0", "cero informa 'ninguna palabra floja' sobre un motor que no mide confianza");
  else if (main) ok("audio.js: `flojas` es null cuando el motor no informa confianza");
}

titulo("colocar_fragmentos: la grilla NO se redondea a 3 decimales (rompe fuera de 25/50fps)");

/*
 * Medido el 2026-09-21 armando dos secuencias a 30fps: `cuadro()` hacia
 * `.toFixed(3)` sobre `Math.round(t/F)*F`, y a 25fps el cuadro mide 0,04 —tres
 * decimales exactos— pero a 30fps mide 0,0333..., asi que el redondeo vuelve a
 * sacar el valor de la grilla. Dejo huecos de 0,01 frame que `revisar` marco.
 *
 * Subframe no es cosmetico en este repo: Premiere recuantiza al primer arrastre y
 * el vinculo video-audio se deduce por rango EXACTO, asi que un clip subframe
 * pierde su audio en el siguiente `editar`.
 */
{
  const ruta = path.join(raiz, "herramientas/colocar_fragmentos.js");
  const src = fs.readFileSync(ruta, "utf8");
  const limpio = src.replace(/\/\*[\s\S]*?\*\//g, "").replace(/^\s*\/\/.*$/gm, "");

  const m = limpio.match(/const cuadro = [^;]+;/);
  if (!m) mal("colocar_fragmentos: no encuentro `cuadro`");
  else if (/toFixed/.test(m[0]))
    mal("colocar_fragmentos: `cuadro` redondea a decimales fijos",
        "exacto solo a 25/50fps; a 30fps saca el valor de la grilla y deja subframe");
  else if (!/Math\.round\([^)]*\/\s*F\s*\)\s*\*\s*F/.test(m[0]))
    mal("colocar_fragmentos: `cuadro` no cuantiza a la grilla de la secuencia");
  else ok("colocar_fragmentos: `cuadro` cuantiza sin redondear a decimales");

  /* Y los out-point de fuente, que se calculan aparte y tenian el mismo patron. */
  const sobran = (limpio.match(/\.dura\)\.toFixed\(/g) || []).length;
  if (sobran) mal("colocar_fragmentos: " + sobran + " calculo(s) de out-point todavia usan toFixed sobre la duracion",
                  "reintroduce el subframe que `cuadro` acaba de evitar");
  else ok("colocar_fragmentos: los out-point de fuente tampoco se redondean");
}

titulo("colocar_fragmentos: --limpiar rebota ANTES de borrar, y su tope sale del panel");

/*
 * Medido el 2026-09-20 rearmando un videoclip de 58 clips por pista: `--limpiar` saco 5, la 6ta
 * llamada rebotó contra la guarda de rafaga de `borrar`, y el chequeo posterior aborto con la
 * pista A MEDIO BARRER. Las dos guardas hicieron lo suyo y el resultado fue el peor posible:
 * cinco clips destruidos para despues negarse.
 *
 * A ~505 ms por llamada el 6to borrado cae a los ~2,5s, muy dentro de la ventana de 60s, asi que
 * un barrido de mas de BORRAR_TOPE clips **no puede terminar nunca**. Se cuenta antes.
 */
{
  const rutaTool = path.join(raiz, "herramientas/colocar_fragmentos.js");
  const rutaPanel = path.join(raiz, "plugin/lib/comandos.js");
  const src = fs.readFileSync(rutaTool, "utf8");
  const limpio = src.replace(/\/\*[\s\S]*?\*\//g, "").replace(/^\s*\/\/.*$/gm, "");
  const panel = fs.readFileSync(rutaPanel, "utf8");

  // 1 — el tope se DERIVA: dos numeros sueltos se desincronizan y nadie lo nota.
  const mPanel = panel.match(/const BORRAR_TOPE\s*=\s*(\d+)/);
  const mTool = limpio.match(/const TOPE_BARRIDO\s*=\s*(\d+)/);
  if (!mPanel) mal("no encuentro `BORRAR_TOPE` en el panel: la guarda del barrido no se puede cotejar");
  else if (!mTool) mal("colocar_fragmentos: no existe `TOPE_BARRIDO`");
  else if (mPanel[1] !== mTool[1])
    mal("colocar_fragmentos: TOPE_BARRIDO (" + mTool[1] + ") no coincide con BORRAR_TOPE del panel (" + mPanel[1] + ")",
        "con el tope alto la herramienta arranca un barrido que `borrar` va a cortar por la mitad");
  else ok("colocar_fragmentos: el tope del barrido es el mismo que el de `borrar` (" + mTool[1] + ")");

  /* El bloque de --limpiar por LLAVES BALANCEADAS. Cortar N caracteres ya hizo fallar tres
   * guardas de este archivo, dos por medir de mas y una por medir de menos. */
  function bloqueDe(txt, ancla) {
    const i = txt.indexOf(ancla);
    if (i === -1) return null;
    let j = txt.indexOf("{", i);
    if (j === -1) return null;
  ...
  const cuerpo = bloqueDe(limpio, 'if (flag("limpiar"))');

  // 2 — el rechazo va ANTES del bucle. Es TODA la correccion: no empezar lo que no se puede terminar.
  if (!cuerpo) mal("colocar_fragmentos: no encuentro el bloque de --limpiar");
  else {
    const iRechazo = cuerpo.indexOf("TOPE_BARRIDO");
    const iBucle = cuerpo.search(/for\s*\(\s*let i = cs\.length/);
    /* Se exige la COMPARACION viva, no que el token aparezca: con `if (false)` el identificador
     * y el `process.exit` siguen ahi y una guarda por presencia pasaria con el rebote desarmado.
     * Es la familia que este repo ya pago cinco veces. */
    const comparaVivo = /cs\.length\s*>\s*TOPE_BARRIDO/.test(cuerpo);
    if (iRechazo === -1 || !comparaVivo) mal("colocar_fragmentos: --limpiar no compara cs.length contra TOPE_BARRIDO");
    else if (iBucle === -1) mal("colocar_fragmentos: no encuentro el bucle de borrado");
    else if (iRechazo > iBucle)
      mal("colocar_fragmentos: el chequeo del tope corre DESPUES de empezar a borrar",
          "asi la pista queda a medio barrer, que es el daño que la guarda existe para evitar");
    else if (!/process\.exit\(1\)/.test(cuerpo.slice(iRechazo, iBucle)))
      mal("colocar_fragmentos: el chequeo del tope no ABORTA", "avisar y seguir barre igual");
    else ok("colocar_fragmentos: --limpiar cuenta y rebota antes de borrar el primer clip");

    // 3 — y si igual falla uno, se corta: 53 intentos mas que rebotan no aportan nada.
    const bucle = bloqueDe(cuerpo.slice(iBucle < 0 ? 0 : iBucle), "for");
    if (bucle && !/catch[\s\S]*?\bbreak\s*;/.test(bucle))
      mal("colocar_fragmentos: el bucle de borrado NO corta en el primer rechazo",
          "sigue intentando con la pista ya a medio barrer");
    else if (bucle) ok("colocar_fragmentos: el barrido corta en el primer rechazo");
  }
}

titulo("Las herramientas MCP RECHAZAN una clave inventada, en vez de descartarla");

/*
 * El panel tiene `PARAMS_DE`, que rebota un parametro que el verbo no lee. **Desde las
 * herramientas MCP era INALCANZABLE**: `inputSchema` se declaraba como un shape de zod, y zod
 * por defecto DESCARTA en silencio lo que no esta declarado, asi que la clave inventada nunca
 * llegaba al panel.
 *
 * Medido con zod 4.4.3: z.object({nombre}).parse({nombre:"X", audio:true}) -> {"nombre":"X"}.
 * Y cobrado tres veces en una sola sesion: `seleccionar` con `audio` selecciono el clip de VIDEO
 * e informo exito, `secuencias` con `activar` contesto como si nada, y una de las tres llevo a un
 * diagnostico equivocado. Es el mismo agujero que ya tuvo la guarda `proyecto`.
 */
{
  const src = fs.readFileSync(path.join(raiz, "server/index.js"), "utf8");
  const limpio = src.replace(/\/\*[\s\S]*?\*\//g, "").replace(/^\s*\/\/.*$/gm, "");

  const conGuarda = (limpio.match(/inputSchema:\s*soloEstas\(\{/g) || []).length;
  const sinGuarda = (limpio.match(/inputSchema:\s*\{/g) || []).length;
  const tools = (limpio.match(/server\.registerTool\(/g) || []).length;

  if (sinGuarda) {
    mal(sinGuarda + " herramienta(s) MCP declaran `inputSchema` como shape pelado",
        "zod DESCARTA en silencio las claves que no estan en el schema, asi que `PARAMS_DE` del panel no las ve");
  } else if (!conGuarda) {
    mal("ninguna herramienta MCP usa `soloEstas`");
  } else if (conGuarda !== tools) {
    mal("hay " + tools + " registerTool y solo " + conGuarda + " con `soloEstas`");
  } else {
    ok("las " + conGuarda + " herramientas MCP rechazan claves que no declararon");
  }

  /* Y que `soloEstas` de verdad ENDUREZCA: sin `.strict()` el helper existe y no hace nada,
   * que es la guarda por presencia que este repo ya pago seis veces. */
  const m = limpio.match(/function soloEstas\(shape\)\s*\{[\s\S]*?\n\}/);
  if (!m) mal("no encuentro `soloEstas` en server/index.js");
  else if (!/\.strict\(\)/.test(m[0]))
    mal("`soloEstas` no llama a `.strict()`", "sin eso devuelve un objeto que descarta igual");
  else ok("`soloEstas` endurece el schema con `.strict()`");
}

titulo("recargar.js: CERO proyectos abiertos no es un fallo");

/*
 * Encontrado usandolo el 2026-09-21. Con Premiere abierto y SIN proyecto, `--reiniciar`
 * contestaba "NO se pudo guardar antes de cerrar: No hay un proyecto abierto · no cierro nada,
 * guardá vos y volvé a intentar" — pidiendo guardar algo que no existe.
 *
 * Es el peor modo de fallo de una guarda: rechazar uso correcto. Y el caso rechazado era el MAS
 * seguro de todos, porque sin proyecto no hay nada que perder ni nada que reabrir.
 *
 * Se decide por el CONTEO de `proyectosAbiertos`, no por el texto del error — juzgar por el
 * mensaje es lo que este repo tiene prohibido desde su primera linea. Y la duda NO se resuelve
 * a favor de cerrar: si el conteo no se pudo leer queda en `null` y el rechazo se mantiene.
 */
{
  const src = fs.readFileSync(path.join(raiz, "herramientas/recargar.js"), "utf8");
  const limpio = src.replace(/\/\*[\s\S]*?\*\//g, "").replace(/^\s*\/\/.*$/gm, "");

  if (!/cuantosAbiertos\s*=\s*abiertos\.length/.test(limpio))
    mal("recargar.js: no registra CUANTOS proyectos habia abiertos");
  else if (!/const sinProyecto = cuantosAbiertos === 0/.test(limpio))
    mal("recargar.js: no distingue 'cero proyectos' de 'el guardado fallo'",
        "sin eso rechaza el caso mas seguro que hay");
  else if (/let cuantosAbiertos = 0/.test(limpio))
    mal("recargar.js: `cuantosAbiertos` arranca en 0 y no en null",
        "un conteo que no se pudo leer pasaria por 'no hay proyectos' y cerraria a ciegas");
  else if (!/if \(!ruta && !sinProyecto\)/.test(limpio))
    mal("recargar.js: aborta por falta de ruta aunque no hubiera proyecto");
  else if (!/ruta \? \[ruta\] : \["-a"/.test(limpio))
    mal("recargar.js: reabre con una ruta que puede ser null",
        "`open` con null tira, y abrir un proyecto que no habia seria peor");
  else ok("recargar.js: sin proyecto abierto reinicia igual, y la duda NO cierra");
}

titulo("Los cuatro defectos reportados por una sesion real (2026-09-21)");

{
  const src = fs.readFileSync(path.join(raiz, "plugin/lib/comandos.js"), "utf8");
  const limpio = src.replace(/\/\*[\s\S]*?\*\//g, "").replace(/^\s*\/\/.*$/gm, "");
  const trans = cuerpoDeFuncion(limpio, "async function transicion");
  const expo  = cuerpoDeFuncion(limpio, "async function exportar");

  /* #2 — el handle se CALCULA y se avisa. El verbo no puede releer la transicion (los items
   * de getTrackItems(TRANSITION) son null, remedido con las tres formas), pero si puede saber
   * de antemano que el fundido pedido no entra. Sin esto informa 1s y salen 0,5. */
  if (!trans) mal("no encuentro `transicion`");
  else if (!/getMedia\(\)/.test(trans))
    mal("`transicion` no lee la duracion del medio", "sin eso no puede calcular el handle: `ClipProjectItem` no expone ninguna duracion");
  else if (!/avisoHandle/.test(trans))
    mal("`transicion` calcula el handle y no lo avisa", "un dato que no esta en el resumen es un dato que no esta");
  else if (!/EL HANDLE NO DA/.test(trans))
  ...
  if (!limp) mal("no existe el verbo `limpiarRangos`");
  else if (!/yaEstabaLimpia: true/.test(limp))
    mal("`limpiarRangos` no distingue 'ya estaba limpia' de 'la limpie'");
  else if (!/No se pudieron leer los in\/out/.test(limp))
...
/*
 * Hasta el 2026-09-23 el CLAUDE.md era regla y bitácora a la vez: 1.222 líneas, y las reglas
 * recién en la línea 1.030. Todo eso se carga en cada sesión, y creció así sin que nada lo
 * frenara. El del repo privado del que sale este llegó a 7.595 líneas.
 *
 * Las propiedades que lo sostienen se rompen todas en silencio:
 *  - el CLAUDE.md no pasa del tope: un caso nuevo va a docs/bitacora/<tema>.md.
 *  - USO.md existe y es corto: una sesión de uso lo lee ENTERO o no sirve.
 *  - todo archivo de docs/bitacora/ figura en el índice: el que no figura, nadie lo va a buscar.
 *  - todo `algo.md` que nombran CLAUDE.md y USO.md existe: un puntero roto manda a leer la nada.
 */
titulo("CLAUDE.md liviano: reglas acá, la bitácora en docs/bitacora/ y USO.md corto");

{
  const TOPE_CLAUDE = 400, TOPE_USO = 250;
  const leer = (rel) => (fs.existsSync(path.join(raiz, rel)) ? fs.readFileSync(path.join(raiz, rel), "utf8") : null);
  const lineas = (s) => s.split("\n").length;
  const claude = leer("CLAUDE.md") || "", uso = leer("USO.md");
  const dirBit = path.join(raiz, "docs/bitacora");
  const bit = fs.existsSync(dirBit) ? fs.readdirSync(dirBit).filter((f) => f.endsWith(".md")) : [];

  if (lineas(claude) > TOPE_CLAUDE)
    mal(`CLAUDE.md volvió a crecer: ${lineas(claude)} líneas, el tope es ${TOPE_CLAUDE}`,
        "un caso nuevo va a docs/bitacora/<tema>.md; acá sube solo una regla que cambia cómo se trabaja, en una línea");
  else ok(`CLAUDE.md tiene ${lineas(claude)} líneas (tope ${TOPE_CLAUDE})`);

  if (!uso) mal("falta USO.md", "es lo que leen las sesiones que USAN el bridge desde otros proyectos");
  else if (lineas(uso) > TOPE_USO)
    mal(`USO.md tiene ${lineas(uso)} líneas, el tope es ${TOPE_USO}`,
        "una sesión de uso tiene que poder leerlo entero de una vez");
  else ok(`USO.md existe y tiene ${lineas(uso)} líneas (tope ${TOPE_USO})`);

  if (!bit.length) mal("docs/bitacora/ está vacía o no existe", "ahí vive la bitácora que salió del CLAUDE.md");
  else {
    const huerfanos = bit.filter((f) => claude.indexOf(f) === -1);
    if (huerfanos.length)
      mal("hay archivos de bitácora que el índice del CLAUDE.md no nombra: " + huerfanos.join(", "),
          "el que no está en el índice por tarea no lo va a ir a buscar nadie");
    else ok(`los ${bit.length} archivos de docs/bitacora/ están en el índice del CLAUDE.md`);
  }
