...
## Movimiento de CÁMARA y foco adentro de un tramo: lo que salió de el evento (2026-09-22)

Dos herramientas del armado de b-roll de un evento corporativo —4K a 59,94, vertical, con gente
caminando por todo el cuadro—, portadas desde el proyecto: **`camara.py`** y **`foco_tramo.py`**,
más **`foco_de_cerca.py`** (las hojas para mirar el foco), **`tira_clip.py`** (un clip entero en una
hoja) y **`video_comun.py`** (geometría y rotación, compartidas). Los originales quedan en el armado,
con su uso real en la `PROPUESTA.md`. El detalle de cada una está en su encabezado.

### `camara.py`: el movimiento del FONDO, no del cuadro

`movimiento.js` —la receta de `MOVIMIENTO_ROTO.md`— mide la diferencia entre cuadros, y su falla
nº1 es que no separa cámara de sujeto. En un institucional la base de cámara en mano quieta daba 0,45–0,63;
en el evento da de **0,74 a 12,25** (mediana 3,31, 54 clips), así que el umbral no se transfiere y
casi todo salía "a mirar".

`camara.py` sigue puntos (`goodFeaturesToTrack` + Lucas-Kanade) y estima una semejanza con RANSAC,
que deja afuera a la gente. El **tirón** es |v − media de ±4 pasos|, en px de una secuencia de 1080 y
con pasos de 40 ms: un paneo parejo da poco aunque se mueva mucho, un golpe da mucho. Con `inl` por
debajo de 0,5 la estimación no vale. Calibrado mirando: **menos de 4 px no se nota, de 4 a 8 es
leve, más de 8 se ve** — y **nunca se probó a ciegas contra el juicio del editor.**

**Al portarla apareció un artefacto que inflaba el tirón.** El original pedía `fps=25` a ffmpeg, y
sobre 59,94 eso no toma un cuadro cada 40 ms: toma cuadros de fuente de a 2 y de a 3 —medido, 30 y
19 veces en dos segundos—, así que un paneo parejo recorre una distancia distinta en cada paso.
Control, con paneos SIN golpes hechos desplazando un cuadro real del evento:

```
                                  fps=25 (original)   portada
paneo 300 px/s, fuente 59,94          3,87              0,33
paneo 600 px/s, fuente 59,94          7,76  "leve"      0,43
paneo 300 px/s, fuente 29,97          9,06  "se ve"     0,27     <- lo normal en un teléfono
paneo 300 px/s, fuente 50             0,38              0,31     50/25 es entero: sin artefacto
GOLPE de 8 px, según su fase       4,98 a 10,46      7,04 a 7,21  lo teórico es 7,1
```

Dos defectos en uno: el paneo parejo lee tirón en proporción a su velocidad, y un golpe real lee la
mitad o una vez y media según caiga en un paso de 2 o de 3 cuadros. La portada mide cada par de
cuadros de fuente con su tiempo real —de `showinfo`, así que un VFR también anda— y toma una
ventana de 40 ms que arranca en cada cuadro. `camara.py control` corre ese control y `test.js` lo
exige.

**Sobre los 64 tramos que se eligieron en el evento**, remedidos con la misma ventana:

```
             mediana    <4    4-8    >8
original       2,19     50     14     0
portada        1,67     60      3     1
```

Los **once** que la propuesta dejó como "movimiento leve, 4 a 7 px" dan entre **1,37 y 3,64**: el
leve era mayormente el artefacto. Los dos que siguen en 4-8 son de los más lentos —A001C113 a
30 px/s, 4,93 → 4,87, y C2426 a 98 px/s, 5,93 → 4,39—, y que ahí los dos métodos casi coincidan es
lo esperable si el artefacto escala con la velocidad.

**Y uno sube sin explicación: C2347, de 3,05 a 4,54**, con el pico a los 10,12 s. No es la fase:
corriendo el original de a un cuadro de fuente lee 2,69 a 3,05 en ese instante. En la tira es un
paneo por el salón con una mujer caminando en primer plano, que puede estar tirando del consenso
(`inl` 0,85), pero eso no se midió. Queda sin causa, y se mira.

**Y un modo de falla que el artefacto tapaba: RANSAC puede seguir al SUJETO.** En A001C114 (el
conector entrando al auto) la portada da 21,85 en el arranque del tramo, con `inl` 0,55: por
arriba del 0,5 que se daba por válido. Mirando la tira se mueven las dos cosas —la cámara acompaña
y el conector entra—, así que se midió de nuevo con los puntos sólo en la tapa:

```
32,40 a 32,70 s    puntos en el cuadro entero    puntos sólo en la tapa
tirón              12 a 46                       0,4 a 4,9
vel                811 -> 266 px/s               596 -> 254 px/s, y sigue frenando hasta 28
inl                0,57 a 0,73                   1,00
```

La cámara hace un paneo que frena parejo; el conector ocupa medio cuadro sobre una chapa lisa, y el
consenso de RANSAC salta entre los dos movimientos. El 3,32 del original sale del mismo cuadro
entero: que caiga cerca de lo que da la tapa no lo vuelve una medición de la cámara.
**`inl` por arriba de 0,5 no garantiza que se haya medido la cámara**: con el sujeto grande y el
fondo liso, se mira, y si hace falta el número, cada clip de la lista acepta `"region"` para buscar
los puntos sólo en el fondo: acá, `[0, 0, 0.3, 1]` da 4,46 con `inl` 0,93.

### `foco_tramo.py`: la guarda que `nitidez.py` describió y nunca se construyó

No mide el foco —una pared lisa puntúa igual que un desenfoque—: mide la nitidez de cada cuadro
**contra el máximo del propio tramo**, donde el contenido casi no cambia. Tenengrad en una grilla de
6x10, percentil 80. En el armado se llamaba `foco.py`; acá no, porque `foco.py` es TOPIQ comparando
tomas entre sí, y se pisaban.

Aciertos, reproducidos con la portada sobre el material:

```
C2391     pantalla que rota logos, fuera de foco a los 2,3 s     cae a 0,05, con vecinos en 0,21
A001C130  folletos                                               0,92 a los 6,77 s -> 0,51 a los 9,34 s
A001C114  el conector se desenfoca entre 8 y 10 s              tramo 8,0-10,1: 0,69 a los 9,13 s, dif 0,7
C2370     detalle del producto, pierde foco a los 2 s                0,98 al arranque -> 0,42 a 1,9 s
```

**Falso positivo medido: C2452 dio 0,34 estando nítido.** Mirando los dos cuadros: el cargador llena
el cuadro en el que mide 0,37 y está chico en el que mide 0,94, y los dos se ven nítidos. La nitidez
medida sigue al TAMAÑO del objeto, y la `dif` entre cuadros consecutivos no lo ve porque el cambio
es lento.

**Y la guarda obvia no sirve, medida para que no se construya.** El contenido del cuadro más nítido
contra el menos nítido da **27,6** en el falso positivo, **4,5 a 16,2** en los cuatro aciertos y
**3,5 a 9,3** en tramos sin problema. Con un solo falso positivo cualquier umbral entre 16 y 27
estaría ajustado a un caso, y la comparación está contaminada de entrada: desenfocarse también
cambia los píxeles.

**Un plano blando de punta a punta no lo ve**: la medida es relativa y su nitidez es pareja. Por
eso en el evento se miraron los 64 tramos con `foco_de_cerca.py`, el cuadro más nítido y el menos
nítido de cada uno. **Y el encabezado del original prometía más que su código**: decía que marcaba
una caída "sólo si la diferencia entre cuadros se mantiene chica", y marcaba por `nitidezMin < 0,75`
o `lumRango > 20`, con la `dif` impresa al lado. La portada lo dice como es; esos dos umbrales no se
calibraron.

Verificado contra el armado: el `foco.json` sale idéntico en tres tramos —`rel`, `dif` y todos los
campos— y la serie de `foco_clips.json`, con diferencia máxima 0,005 sobre valores de ~190.

### La rotación, que esta vez estaba BIEN

El evento viene 3840x2160 con flag ±90 y el contenido es vertical de verdad; en un videoclip y un institucional el
flag mentía. `familias.py` y `grilla_angulos.js` lo ignoran por default porque se escribieron sobre
un videoclip, y con ese default estas herramientas habrían medido todo el evento acostado —y el tirón, 1,78
veces más chico, sin un error—. Así que **`video_comun.py` no tiene default**: si un clip trae flag y
no se dijo qué hacer, rebota antes de medir, y se decide mirando `tira_clip.py` con las dos opciones.
Sin flag no pide nada.

### Lo que queda abierto

- **Las bandas de 4 y 8 px se calibraron sobre los números del artefacto**, y nada de esto se probó
  a ciegas contra el editor. Antes de usarlas como corte con la portada, hay que volver a mirar.
