 ...
 *
 *   3 de 3 corridas ENTERAS limpias sobre una COPIA de un videoclip y la propuesta
...
/*
 * LEER UNA PISTA PARA LIMPIARLA NO PUEDE FALLAR EN SILENCIO (2026-09-11).
 *
 * Esto era `try { … } catch (e) { cs = []; }` en tres lugares: un fallo de lectura se
 * convertia en lista vacia, la herramienta informaba "0 clip(s)" y seguia como si no
 * hubiera nada que borrar. Visto en vivo con la pista teniendo 88 — el contador ciego
 * del CLAUDE.md, con una lectura en vez de un conteo.
 *
 * Y el daño no es informar mal: es que despues se COLOCA encima de una pista que no se
 * pudo inspeccionar. Una lectura fallada no es una pista vacia, asi que se aborta.
 *
 * Ademas va con la guarda de secuencia, que le faltaba mientras el `borrar` de la linea
 * siguiente si la tenia: leer de una secuencia y borrar en otra es la mitad de un
 * desastre, y que la guarda del `borrar` lo atajara era suerte, no diseño.
 */
async function clipsDe(pista) {
  try {
    return (await enviar("clips", g({ pista }))).clips || [];
  } catch (e) {
    console.error("\nNo se pudo LEER " + pista + " para limpiarla: " +
                  String((e && e.message) || e).slice(0, 140));
    console.error("NO se borro ni se coloco nada: una lectura fallada no es una pista vacia.");
    process.exit(1);
  }
}
...
(async () => {
  ...
  if (maxV > hayV || necA > hayA) {
    console.error("\nFaltan pistas, y tienen que existir ANTES de colocar: `insertar` no crea las de video,");
  ...
  if (LIMPIAR_PISTA) {
    ...
    const quedan = (await clipsDe("V" + v)).length;
  ...
  if (LIMPIAR) {
    /*
     * EL VEREDICTO ES RELEER, NO CONTAR LOS QUE NO TIRARON (2026-09-11).
     *
     * Antes los errores de `borrar` se tragaban con un catch vacio —"se informa al final
     * por el conteo"— y el conteo era `saco`, que sube cuando la llamada NO TIRA. O sea
     * que informaba exito sobre transacciones que podian no haber corrido, y despues
     * colocaba encima. `--limpiar-pista` ya releia y exigia cero; esto no.
     *
     * Es el "no se juzga por el MENSAJE, se juzga por el ESTADO" del CLAUDE.md, con el
     * agravante de que la rama destructiva sigue igual: colocar sobre una pista que no
     * se vacio deja el corte mezclado con lo viejo.
     */
    let saco = 0;
    const fallados = [];
    const pistas = [];
    for (let v = 1; v <= hayV; v++) pistas.push("V" + v);
    /* Audio de A2 en adelante: A1 NO se toca, ahí va el tema. */
    for (let aa = 2; aa <= hayA; aa++) pistas.push("A" + aa);

    for (const pista of pistas) {
      const cs = await clipsDe(pista);
      for (let i = cs.length - 1; i >= 0; i--) {
        await dormir(PAUSA);
        try { await enviar("borrar", g({ pista, indice: i, dejarHueco: true }), 300000); saco++; }
        catch (e) { fallados.push(pista + "[" + i + "]: " + String((e && e.message) || e).slice(0, 70)); }
      }
      if (cs.length) console.log("  " + pista + ": " + cs.length + " fuera");
      await dormir(PAUSA);
    }

    const sucias = [];
    for (const pista of pistas) {
      const n = (await clipsDe(pista)).length;
      if (n) sucias.push(pista + " (" + n + ")");
    }
    console.log("  " + saco + " borrado(s)" +
                (fallados.length ? " · " + fallados.length + " fallaron" : "") + "\n");
    if (fallados.length) for (const x of fallados.slice(0, 5)) console.log("    ✗ " + x);
    if (sucias.length) {
      console.error("NO quedaron vacias: " + sucias.join(", ") + ". No sigo: colocar encima");
      console.error("dejaria el corte nuevo mezclado con lo viejo.");
      process.exit(1);
    }
  }
  ...
  for (const t of (SIN_LOTE ? tareas : [])) {
    ...
    try {
      ...
      /* El veredicto es `entro`, NO la palabra "quedó": el resumen la trae tambien en el FANTASMA
       * —un clip del mismo medio que ya estaba en ese segundo, con la insercion sin entrar— y los
       * `editar` de abajo recortarian el clip viejo. Ver colocar_sincro.js, mismo arreglo. */
      if (r1.entro !== true) throw new Error(String(r1.resumen).slice(0, 120));
