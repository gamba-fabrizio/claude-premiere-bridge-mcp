...
/* EL TOPE DEL BARRIDO ES EL DE LA GUARDA DE `borrar`, QUE VIVE EN EL PANEL.
 *
 * Esta escrito acá porque la herramienta no puede preguntarle al panel cuanto vale su constante,
 * pero NO se puede desincronizar: `test.js` lee `BORRAR_TOPE` de `plugin/lib/comandos.js` y exige
 * que sean el mismo numero. Dos topes sueltos se suben en uno y no en el otro, y nadie lo nota
 * hasta que una tanda deja la pista a medio barrer — que es exactamente el daño que esto evita. */
 ...
 *
 * SIN `.toFixed(3)`, y eso es el arreglo de un bug medido el 2026-09-21. Redondear a tres
 * decimales es exacto SOLO cuando el cuadro tiene tres decimales: a 25fps mide 0,04 y a 50fps
 * 0,02, asi que durante meses no se noto. A **30fps el cuadro mide 0,0333...** y el redondeo
 * vuelve a sacar el valor de la grilla: sobre las dos secuencias del evento dejo huecos de
 * 0,01 frame que `revisar` marco. El double ya representa el frame mas cerca de lo que Premiere
 * puede distinguir; recortarlo a tres decimales solo agrega error. */
const cuadro = (t) => Math.round(t / F) * F;
...
(async () => {
  ...
  if (flag("limpiar")) {
    ...
    /* `--limpiar` NO PUEDE BARRER UNA PISTA LARGA, y rebota ANTES de borrar el primero.
     *
     * `borrar` tiene en el PANEL una guarda de rafaga que rechaza a partir del 5to borrado en
     * una ventana de 60s, porque barrer de a uno ya tiro Premiere dos veces (176 clips con
     * solapes, y 8 seguidos). Este bucle corre a ~505 ms por llamada, asi que el 6to cae a los
     * ~2,5s: **un barrido de mas de TOPE_BARRIDO clips no puede terminar, nunca.**
     *
     * Medido el 2026-09-20 rearmando el videoclip de un videoclip, 58 clips por pista: saco 5, la 6ta
     * rebotó, y el chequeo de "quedan" aborto con la pista A MEDIO BARRER. Las dos guardas
     * hicieron lo suyo y el resultado igual fue el peor posible — destruir 5 clips para despues
     * negarse. Empezar algo que no se puede terminar es peor que no empezarlo.
     *
     * Asi que se cuenta primero. Y lo que se propone no es "reintentá": es la operacion que SI
     * sirve, que es UNA destructiva en vez de N. */
    if (cs.length > TOPE_BARRIDO) {
      console.error(`\nV${PISTA} tiene ${cs.length} clip(s) y --limpiar NO puede barrerla.`);
      console.error(`\`borrar\` rebota a partir del ${TOPE_BARRIDO + 1}er borrado en 60s, y este bucle llega ahi en ~3s.`);
      console.error("Si igual se intenta, la pista queda A MEDIO BARRER: ya paso.\n");
      console.error("  lo que SI funciona, y es UNA operacion destructiva en vez de " + cs.length + ":");
      console.error(`    1. borrarSecuencia "${SEC}"  +  armarSecuencia con los mismos ajustes`);
      console.error("    2. o a mano en Premiere: seleccionar la pista y apretar Delete");
      console.error("       (nativo, instantaneo y sin riesgo)");
      console.error(`    3. o armar en una pista vacia:  --pista <n>\n`);
      process.exit(1);
    }
    ...
    for (let i = cs.length - 1; i >= 0; i--) {
      await dormir(PAUSA);
      /* Se CORTA en el primer rechazo en vez de seguir. Con 58 clips, seguir eran 53 intentos
       * mas que rebotan igual: ruido, y ~27s de espera para terminar en el mismo lugar. */
      try { await enviar("borrar", g({ pista: "V" + PISTA, indice: i, dejarHueco: true }), 300000); saco++; }
      catch (e) {
        console.log(`   ✗ no se pudo borrar [${i}]: ${String(e.message || e).slice(0, 80)}`);
        break;
      }
    }
  ...
  if (porLote.length) {
    ...
    const frags = porLote.map((p) => ({
      ...
      /* El out-point de fuente NO se redondea a 3 decimales: `dura` ya viene en la grilla
       * de la secuencia y recortarlo la vuelve a sacar fuera de 25/50fps. Ver `cuadro`. */
      desde: p.entrada, hasta: p.entrada + p.dura
    }));
    try {
  ...
  for (let i = 0; i < porPaso.length; i++) {
    ...
    try {
      ...
      /* Por el CAMPO y no por la frase: hoy "NO SE PUSO NADA" sale exactamente cuando `entro` es
       * false, pero una frase se reescribe y un campo no. Colocar_sincro y colocar_propuesta
       * validaban por la palabra "quedó", y esa sale tambien en el fantasma. */
      if (r.entro !== true) {
      ...
      const pedido = { pista: "V" + PISTA, indice: mio.indice,
                       salida: entQuiero + p.dura };  // sin toFixed: ver `cuadro`
