...
(async () => {
   ...
   * tiene que quedar con su audio en SU pista. */
  ...
  for (const t of tramos) {
    ...
    try {
      ...
      /* El veredicto es `entro`, NO la palabra "quedó". El resumen dice "quedó en V…" apenas hay
       * un clip en ese segundo, TAMBIEN cuando es el que ya estaba y la insercion no entro (el
       * fantasma): validar por la palabra dejaba pasar justo ese caso, y los `editar` de abajo
       * recortaban y movian el clip VIEJO. Medido el 2026-09-22: con el fantasma, el resumen trae
       * "quedó" y `entro` es false. */
      if (r1.entro !== true) throw new Error(String(r1.resumen).slice(0, 150));
      if (t.ent > 0) {
        await dormir(PAUSA);
