...
async function exportar(params) {
  ...
  /*
   * LOS MODOS DE COLA ENCOLAN Y NO ARRANCAN, y eso convertia `ame` en una trampa.
   *
   * Encolar devuelve al instante y el item queda esperando que alguien apriete play en
   * Media Encoder. Desde este lado es indistinguible de un export que esta tardando: se
   * perdieron cuatro minutos esperando un archivo que no iba a empezar a escribirse nunca,
   * y volvio a pasar en otra sesion.
   *
   * `startBatchEncode()` la arranca, y la receta esta MEDIDA desde el 2026-09-11 con el
   * control que la hace valer: 30 segundos sin tocar nada -> NO se escribio nada; despues
   * `launchEncoder()` + `startBatchEncode()` -> archivo en disco a los 10s. Sin esos 30
   * segundos de nada, el archivo apareciendo no distinguiria "lo arranco la llamada" de
   * "tardo en arrancar solo". Quedo sin cablear "hasta que aparezca un caso real"; aparecio.
   *
   * `launchEncoder` va primero porque `startBatchEncode` necesita AME abierto, y se tolera
   * que falle: si ya estaba abierto no hace falta. Lo que NO cambia es la confirmacion —el
   * archivo lo escribe otro proceso, asi que el verbo sigue sin poder mirarlo y lo dice.
   */
  let arranque = null;
  if (modo !== "ya") {
...
/**
 * Saca los in/out de la SECUENCIA, que es el Opt+X que `estado` manda a hacer a mano.
 *
 * POR QUE HACIA FALTA. `estado` y `exportar` detectan una marca vieja y avisan bien
 * —"eso RECORTA el export, Opt+X los saca"— y despues **nada podia sacarla**: las dos
 * fabricas existen pero solo vivian adentro de `exportar`, que las usa para poner un
 * rango y reponerlo. O sea un aviso sin remedio. Freno un trabajo real el 2026-09-21 y
 * hubo que hacerlo a mano en dos secuencias.
 *
 * Y NO hace falta un `createClearInOutPointsAction`, que es lo que el comentario de
 * `exportar` daba por bloqueante. Sin marca los getters devuelven un SENTINEL —-400000,
 * medido sobre cuatro secuencias— y ese sentinel **se puede reescribir**: el export de
 * un multicamara lo repuso y la relectura volvio a dar -400000. Limpiar es escribirlo.
 *
 * LAS DOS EN UNA TRANSACCION, que es la forma medida. `exportar` PONE el rango en dos
 * transacciones separadas y con el in primero, porque el estado intermedio deja un rango
 * invertido que Premiere descarta en silencio; pero para REPONER las manda juntas y
 * funciona. Aca los dos van al mismo valor absurdo, asi que no hay orden que respetar.
 *
 * El veredicto sale de RELEER, no del booleano: se comprueba que los dos getters vuelvan
 * a dar el sentinel, y si no, se dice que no se limpio en vez de darlo por bueno.
 */
async function limpiarRangos() {
  const { project, sequence } = await getProyectoYSecuencia();
  const SENTINEL = -400000;
  const esSentinel = (s) => Math.abs(s) > 1e5;

  let antes = null, leerError = null;
  try {
    antes = { desde: aSegundos(await sequence.getInPoint()),
              hasta: aSegundos(await sequence.getOutPoint()) };
  } catch (e) { leerError = e && e.message ? e.message : String(e); }

  /* Un fallo de LECTURA no es "no tenia marca". Informar que ya estaba limpia sobre una
   * secuencia que no se pudo mirar es el falso negativo que este archivo paga seguido. */
  if (!antes) {
    throw new Error(`No se pudieron leer los in/out de "${sequence.name}": ${leerError}. NO se tocó nada.`);
  }

  if (esSentinel(antes.desde) && esSentinel(antes.hasta)) {
    return {
      resumen: `"${sequence.name}": NO CAMBIÓ NADA, ya no tenía in/out ` +
               `(los getters dan el sentinel ${antes.desde}/${antes.hasta}). No se ejecutó ninguna transacción.`,
      secuencia: sequence.name, yaEstabaLimpia: true, antes: antes, despues: antes, limpio: true
    };
  }
  ...
  } catch (e) { escribirError = e && e.message ? e.message : String(e); }

  let despues = null;
  try {
    despues = { desde: aSegundos(await sequence.getInPoint()),
                hasta: aSegundos(await sequence.getOutPoint()) };
  } catch (e) { despues = null; }

  const limpio = !!despues && esSentinel(despues.desde) && esSentinel(despues.hasta);
  return {
    resumen:
      `"${sequence.name}": in/out ${antes.desde.toFixed(2)}–${antes.hasta.toFixed(2)}s → ` +
      (limpio ? `SIN MARCA (sentinel ${despues.desde}/${despues.hasta}) · un Cmd+Z los devuelve`
              : `NO SE LIMPIARON: quedaron en ${despues ? despues.desde.toFixed(2) + "–" + despues.hasta.toFixed(2) : "no se pudieron releer"}` +
                (escribirError ? ` · ${escribirError}` : ok ? " · la transacción devolvió true igual" : " · la transacción devolvió false")),
    secuencia: sequence.name, yaEstabaLimpia: false,
    antes: antes, despues: despues, limpio: limpio, transaccion: ok, error: escribirError
  };
}
...
async function abrirProyecto(params) {
  ...
  /*
   * `sinDialogos` apaga los modales de apertura (2026-09-11).
   *
   * `OpenProjectOptions` tiene `setShowLocateFileDialog`, `setShowWarningDialog` y
   * `setShowConvertProjectDialog`. El primero es el que importa: es el "Missing media for
   * these clips" que sale cuando al proyecto le faltan medios —por ejemplo, proxies
   * borrados al terminar el trabajo—.
   *
   * Y ese modal no es cosmetico: BLOQUEA EL Cmd+Q, con lo cual `recargar.js --reiniciar`
   * se cuelga 60s y termina culpando al macro de Keyboard Maestro, que esta bien. Medido
   * el 2026-09-11 con un proyecto al que se le borro el archivo de un medio a proposito:
   * con las opciones puestas abrio sin un solo modal, con el medio offline.
   *
   * NO ES EL DEFAULT a proposito. Suprimir el aviso de medios faltantes tambien lo esconde
   * de alguien que necesitaba verlo: abrir a mano tiene que seguir avisando. Esto es para
   * tandas desatendidas, donde un modal no lo ve nadie y ademas traba todo lo que sigue.
   */
  let opciones = null, estadoOpciones = "no se pidieron";
  if (params.sinDialogos === true) {
    try {
      opciones = new ppro.OpenProjectOptions();
      const puestas = [];
      for (const [m, etq] of [["setShowLocateFileDialog", "medios faltantes"],
                              ["setShowWarningDialog", "avisos"],
                              ["setShowConvertProjectDialog", "conversión"]]) {
        if (typeof opciones[m] === "function") { opciones[m](false); puestas.push(etq); }
      }
      estadoOpciones = puestas.length ? "apagados: " + puestas.join(", ") : "OpenProjectOptions no expone ningún setter";
      if (!puestas.length) opciones = null;
    } catch (e) {
      opciones = null;
      estadoOpciones = "NO SE PUDIERON PONER: " + String((e && e.message) || e).slice(0, 80);
    }
  }

  const formas = opciones ? [
    ["open(ruta, opciones)", () => ppro.Project.open(ruta, opciones)],
    ["open(ruta) SIN opciones", () => ppro.Project.open(ruta)],
  ] : [
  ...
  return {
    resumen:
      ...
      /* El estado de los diálogos va en el RESUMEN: si se suprimió el aviso de medios
         faltantes, quien lea tiene que enterarse ahí y no en un campo que nadie mira. */
      (params.sinDialogos === true ? ` · diálogos: ${estadoOpciones}` : "") +
...
/*
 * EL NIVEL DE UN CLIP DE AUDIO NO VIENE EN dB (medido el 2026-09-26). `Volume > Level` es la escala
 * CRUDA de Premiere, crudo = 10^((dB − 15)/20): 0 dB es 0,1778 y el tope, 1, es +15 dB. Medido de
 * AFUERA —cinco tramos de un tono exportados a WAV y medidos con ffmpeg—: los crudos de −6, +6, −18 y
 * +15 dB salieron −6,0, +6,0, −18,0 y +15,0 dB contra el 0,1778 de fábrica, que sonó igual que el
 * archivo. Leído como lineal, 0,1778 parece −15 dB, y una sesión se lo informó así al editor
 * (un reporte de uso, 2026-09-25); escrito en dB, −6 se clampea a 0 y el clip queda MUDO. Solo este param:
 * `Channel Volume` quizás use la misma escala, y no está medido.
 */
const esNivelDeAudio = (efecto, param) =>
  /^volume$/i.test(String(efecto || "")) && /^level$/i.test(String(param || ""));
const crudoADb = (c) => (typeof c === "number" && c > 0 ? 20 * Math.log10(c) + 15 : -Infinity);
const dbACrudo = (db) => Math.pow(10, (db - 15) / 20);
const textoDb = (c) => {
  const d = crudoADb(c);
  if (!isFinite(d)) return "-inf dB, MUDO";
  const r = Math.round(d * 10) / 10;
  return `${(r === 0 ? 0 : r).toFixed(1)} dB`;   // sin «-0.0»: el crudo de fábrica da -0,0000001
};
const ESCALA_NIVEL = "Level es CRUDO: 0 dB = 0.1778 y 1 = +15 dB; en dB se pide con `db`";
/* Lo que se pide para Level, validado ANTES de escribir: `db` explícito, o `valor` crudo en su rango.
   Un crudo fuera de 0..1 nunca es lo que se quiso —negativo es el clip MUDO—, así que rebota. */
function nivelPedido(e, donde) {
  if (e.db !== undefined) {
    if (typeof e.db !== "number" || !isFinite(e.db)) throw new Error(`${donde}: \`db\` tiene que ser un número. NO se escribió nada.`);
    if (e.valor !== undefined) throw new Error(`${donde}: pasá \`db\` o \`valor\`, no los dos (con Level, \`valor\` es el número CRUDO). NO se escribió nada.`);
    if (e.db > 15) throw new Error(`${donde}: Level llega a +15 dB (crudo 1) y se pidió ${e.db} dB. NO se escribió nada.`);
    return dbACrudo(e.db);
  }
  if (typeof e.valor === "number" && (e.valor < 0 || e.valor > 1)) {
    throw new Error(`${donde}: Volume > Level es CRUDO, de 0 a 1 —0 dB es 0.1778 y 1 es +15 dB—, y vino ${e.valor}. ` +
      `Para pedirlo en dB: \`db: ${e.valor}\`. Con ${e.valor} crudo el clip quedaría ${e.valor < 0 ? "MUDO" : "en +15 dB"}. NO se escribió nada.`);
  }
  return e.valor;
}
...
/* El aviso en palabras, que cambia con el tipo: una pista de video NO SE VE y una de
   audio NO SE OYE. Decir "muteada" de un ojito manda a mirar el lugar equivocado. */
function avisoDeSalida(etiqueta, estado, cuantos) {
  if (estado === true) {
    return etiqueta + " (" + cuantos + " clip(s)) " +
      (etiqueta[0] === "V" ? "TIENE LA SALIDA APAGADA: los clips estan pero NO SE VEN"
                           : "esta MUTEADA: los clips estan pero NO SE OYEN");
  }
  return etiqueta + " (" + cuantos + " clip(s)): estado de salida " + estado;
}
...
async function clips(params) {
    ...
    for (let t = 0; t < g.cuantas; t++) {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        salida.push({
          ...
          /*
           * EL OJITO DEL CLIP. Faltaba, y el hueco no era cosmetico: `desactivar` lo
           * ESCRIBE y este verbo --que es con el que uno mira una pista-- no lo devolvia,
           * asi que era write-only. Y quien lo leyera de la fila obtenia `undefined`, que
           * es falsy: contar `filter(x => x.desactivado)` daba CERO sobre una pista entera
           * deshabilitada. Paso el 2026-09-17 sobre la locucion en español de un pitch, y
           * mando a investigar por que no sonaba algo que estaba apagado a la vista.
           */
          desactivado: await ojito(items[i]),
  ...
  const off = salida.filter((c) => c.desactivado === true);
  const sinLeer = salida.filter((c) => c.desactivado === null);
  ...
  return {
    resumen:
      ...
      /* Y los clips con el OJITO apagado, por el mismo motivo: estan en la pista, salen
         en la lista con su tiempo y su duracion, y NO SE VEN. Sin esto en el resumen, una
         pista entera deshabilitada se lee igual que una sana. */
      (off.length
        ? ` · OJO: ${off.length} de ${salida.length} clip(s) tienen el OJITO APAGADO y NO SE VEN: ` +
          off.slice(0, 6).map((c) => `${c.pista}[${c.indice}]`).join(" ") +
          (off.length > 6 ? ` y ${off.length - 6} mas` : "")
        : "") +
      (sinLeer.length
        ? ` · en ${sinLeer.length} clip(s) NO se pudo leer el ojito (van con desactivado:null)`
        : ""),
    clips: salida,
    pistasApagadas: apagadas,
    clipsDesactivados: off.map((c) => `${c.pista}[${c.indice}]`)
  };
}
...
async function param(params) {
  ...
  const nivel = esNivelDeAudio(nombreEfecto, params.param);
  const salida = {
    clip: nombreClip,
    efecto: nombreEfecto,
    param: params.param,
    ...
    valores: nivel ? enKeyframes.map((k) => Object.assign({}, k, { db: typeof k.valor === "number" ? Number(crudoADb(k.valor).toFixed(2)) : null })) : enKeyframes
  };
  if (nivel && numero !== null) salida.db = Number(crudoADb(numero).toFixed(2));
...
async function armarSecuencia(params) {
  ...
  for (let i = 0; i < capas.length; i++) {
    ...
    try {
      ...
      const lugares = [
        { etiqueta: "V" + (pistaIndex + 1), audio: false, traer: () => nueva.getVideoTrack(pistaIndex) },
        { etiqueta: "A" + (pistaAudioIdx + 1), audio: true, traer: () => nueva.getAudioTrack(pistaAudioIdx) }
      ];
      let ok = false;
  ...
  for (const c of capasPuestas) {
    try {
      const idx = Number(String(c.pista).slice(1)) - 1;
      const track = /^A/.test(c.pista) ? await nueva.getAudioTrack(idx) : await nueva.getVideoTrack(idx);
...
/**
 * El nombre de un item del panel, o null si no se puede leer.
 *
 * `importar` se cayo con "Cannot read properties of null (reading 'name')" sobre un proyecto de
 * 318 medios en 4 bins, y la tanda entera rebo­to sin colocar nada —que es lo correcto, pero el
 * trabajo quedo parado—. La causa: los tres recorridos de bins hacian `String(hijos[i].name)` a
 * secas, y un item nulo en la lista voltea la llamada.
 *
 * Que la API devuelva un hueco en una lista no esta documentado y no se pudo reproducir; lo que
 * si se puede es no morirse por eso. **Y los salteados se CUENTAN y se informan**: un recorrido
 * que se come items en silencio contesta "no esta" sobre algo que si estaba, que es el falso
 * negativo que este archivo ya pago con las transiciones, los muteados y los efectos del master.
 */
...
async function insertar(params) {
   ...
   *
   * El testigo del overwrite exacto se habia escrito SOLO para video. Un medio SIN video que
   * pisa exacto a otro audio del mismo largo no hace crecer ninguna pista, y el verbo
   * contestaba "NO SE PUSO NADA, ni en video ni en audio" con el clip nuevo adentro. Medido el
   * 2026-09-22 reemplazando la musica del evento, dos veces seguidas. Es el falso negativo del
   * .wav del tema que vuelve por la pista que el arreglo anterior no cubrio.
   */
  let nombreAntesEnPuntoA = null;
  for (let i = 0; itemsAntesA && i < itemsAntesA.length; i++) {
    if (Math.abs(aSegundos(await itemsAntesA[i].getStartTime()) - segundos) < 0.05) {
      nombreAntesEnPuntoA = String(await itemsAntesA[i].getName());
      break;
    }
  }
  ...
  /* Lo mismo en la pista de AUDIO: es lo unico que ve el overwrite exacto de un medio sin
     video. Con el MISMO nombre antes y despues no cuenta: seria el que ya estaba. */
  const cambioEnPuntoA = puestoA.length > 0 && puestoA[0].nombre !== nombreAntesEnPuntoA;
  const entro = crecioV || crecioA || audioPuesto > 0 || cambioEnPunto || cambioEnPuntoA;
  ...
  /* El fantasma de AUDIO sólo se avisa cuando no hay video en juego: con video, el aviso de
     arriba ya lo dice, y dos avisos del mismo hecho se leen como dos problemas. */
  const fantasmaA = !puesto.length && puestoA.length > 0 && !crecioA && !cambioEnPuntoA && !(audioPuesto > 0);
  ...
  return {
    resumen:
      ...
      (fantasma
        ...
        : "") +
      /* "PISO" y no "PISO EXACTO": el mismo conteo sale tambien cuando el nuevo es MAS LARGO y
         tapa al viejo entero —medido el 2026-09-22, un medio de 39s sobre un clip de 5s—. Y si
         el conteo BAJA, el overwrite se llevo mas de un clip: eso se dice, porque decir "el
         conteo no se mueve" ahi seria mentir sobre lo que se borro. */
      (cambioEnPuntoA && !crecioA
        ? (nombreAntesEnPuntoA !== null
          ? ` · en ${etqA} PISO a "${nombreAntesEnPuntoA}"` +
            (despuesA < antesA
              ? ` y se llevo ${antesA - despuesA} clip(s) mas ENTEROS (${etqA}: ${antesA} → ${despuesA}): el overwrite borra lo que tapa`
              : ` (lo tapa entero: el conteo no se mueve, es un reemplazo)`)
          : ` · en ${etqA} el conteo no crecio pero ahora arranca "${puestoA[0].nombre}" en ese segundo: piso lo que cubria`)
        : "") +
      /* La conclusion tiene que ser la MISMA que la del cierre: `colocar_fragmentos.js` decide por
         el "NO SE PUSO NADA" de abajo, y un aviso que diga "no se sabe" al lado de uno que dice
         "no entro" se lee como dos veredictos. */
      (fantasmaA
        ? ` · OJO: en ${etqA} hay un clip en ese segundo, pero es "${puestoA[0].nombre}", el MISMO que ` +
          `ya estaba, y ${etqA} NO crecio (${antesA} → ${despuesA}): no se distingue de una insercion ` +
          `que no entro, asi que se da por NO puesta. NO lo edites creyendo que es el nuevo`
        : "") +
    ...
    crecioAudio: crecioA,
    fantasma: fantasma,
    fantasmaAudio: fantasmaA,
    nombreAntesEnPunto: nombreAntesEnPunto,
    nombreAntesEnPuntoAudio: nombreAntesEnPuntoA,
    transaccion: ok,
    excepcion: excepcion
  };
}
 ...
 *
 * Por eso ahora se exige que el socio sea del OTRO tipo.
 *
...
/*
 * LO QUE BORRA POR DENTRO, Y NO ES `borrar` (2026-09-25). `vaciarSecuencia`, `unirAudio` y `unirVideo`
 * sacan clips con `createRemoveItemsAction` sobre la selección VIVA, y la dejaban con los clips
 * borrados adentro: el patrón que tiró Premiere en `borrar` el 2026-09-24 —la edición siguiente
 * dispara el aviso de selección y Effect Controls lee un clip que ya no existe—. Y los dos `unir*`
 * encima estiran el clip de al lado en la transacción siguiente, que es exactamente esa edición.
 *
 * `vaciarSeleccion` hace lo que `borrar` ya medía bien —6 de 6 sin crash—: saca los items, fija la
 * selección, relee que quedó vacía y espera. `borradoPermitido` y `anotarBorrado` los hacen contar
 * en la MISMA ventana que `borrar`: `unirAudio` barre todas las pistas, y barrer borrando es lo que
 * tiró Premiere dos veces. Se cuenta lo que salió, no lo intentado.
 */
async function vaciarSeleccion(sequence, seleccion, items) {
  let quedo = false;
  try {
    for (const it of items) {
    ...
    quedo = (await seleccion.getTrackItems()).length === 0;
  } catch (e) { quedo = false; }
  await esperarEntreTx();
  return quedo;
}

function borradoPermitido() {
  const ahoraMs = Date.now();
  while (BORRADOS_RECIENTES.length && ahoraMs - BORRADOS_RECIENTES[0] > BORRAR_VENTANA_MS) BORRADOS_RECIENTES.shift();
  if (BORRADOS_RECIENTES.length < BORRAR_TOPE) return { ok: true, libreEn: 0 };
  return { ok: false, libreEn: Math.ceil((BORRAR_VENTANA_MS - (ahoraMs - BORRADOS_RECIENTES[0])) / 1000) };
}
 ...
 */
/*
 * El OJITO de un clip. Vive en un helper y no inline porque lo leen DOS verbos
 * --`clips` y `radiografia`-- y este mismo archivo tiene escrito, en el comentario de
 * `clips`, que dos caminos para la misma pregunta se separan siempre.
 *
 * DEVUELVE `null` CUANDO NO SE PUDO LEER, nunca `false`. `false` significa "se ve", asi
 * que tragarse el error devolviendo false informaria como activo un clip que no se pudo
 * mirar -- el mismo lado por el que se pago el cuadro negro de la pista muteada.
 */
async function ojito(clip) {
  try {
    const v = await clip.isDisabled();
...
async function estado() {
  ...
  return {
     ...
     * la secuencia se llamaba "NOMBRE DEL PROYECTO ROUGH CUT CLAUDE", así que la
     * confusión pasaba desapercibida.
     *
     * Costó tres llamadas el 2026-08-22: Premiere había cambiado el foco al proyecto de prueba
     * y yo interrogaba `medios` y `bins` creyendo que contestaban sobre el de un corporativo. Fueron
...
async function motion() {
   ...
   *
   * Apareció leyendo el nest animado de VIDEO 1 en el proyecto de un corporativo: 20 keyframes de
...
async function keyframe(params) {
  ...
  const pedidos = Array.isArray(params.lista) && params.lista.length
    ? params.lista
    : [{ segundos: aSegundos(playhead), valor: params.valor, db: params.db, x: params.x, y: params.y }];
  const nivel = esNivelDeAudio(nombreEfecto, nombreParam);

  const puntos = [];
  for (let i = 0; i < pedidos.length; i++) {
    const e = pedidos[i];
    if (e.db !== undefined && !nivel) {
      throw new Error(`\`db\` es solo para Volume > Level; "${nombreEfecto} > ${nombreParam}" se escribe con \`valor\`. NO se escribió nada.`);
    }
    const v = nivel ? nivelPedido(e, `el keyframe ${i}`) : e.valor;
    let valor;
...
async function vaciarSecuencia(project, sequence) {
  ...
  await vaciarSeleccion(sequence, seleccion, items);
 ...
 * de 6 streams puesta sin audio trajo SEIS clips de audio a A2–A7, prendidos y sonando encima del
 * switch, y la cola volvió con el nombre del medio. Reproducido en el proyecto de prueba, donde
 * además perdió su Lumetri. Y por el mismo camino, lo que el reporte no vio:
 *
 *   - el audio iba a las pistas que ESPEJAN el video, lo tuviera o no el original, y el overwrite
 *     PISA: con algo en esas pistas en ese tramo, se borraba sin aviso;
 *   - la cola nacía con los componentes por defecto: sin efectos, sin el Essential Sound ni el
 *     volumen de su audio, a 1x y prendida. Solo Scale y Position se reponían.
 *
 * El clon, medido ese día con las tres formas de la firma: es UN item —clonar el video no trae su
 * audio— y trae el nombre, el apagado y los efectos con sus valores. Un clip de audio se clona a su
 * misma pista. Así que el grupo se arma clonando cada socio, y la cola tiene exactamente el audio
 * que tenía el original, en las mismas pistas. `createSetStartAction` recorta el borde izquierdo en
 * tiempo de secuencia y Premiere calcula la entrada; `createMoveAction` toma un delta.
 *
 * EL COSTO, y se dice en cada corte que lo paga: **la cola queda SIN VÍNCULO en Premiere.** La API
 * no crea vínculos —no hay un método en ninguna de sus 71 clases— y un clon nace suelto: leído en el
 * `.prproj` guardado, que es donde Premiere guarda los grupos. El bridge los sigue emparejando por
 * medio y rango, y a mano se vuelven a vincular con Cmd+L. El editor lo eligió antes que perder el
 * procesamiento de la cola.
 *
 * `SEQUENCE_OPERATION_APPLYCUT` no sirve para esto: es qué hacer con los cortes que encuentra la
 * detección de escenas, no un corte en un tiempo cualquiera.
 */
async function cortar(params) {
  ...
  /*
   * A OTRA VELOCIDAD NO SE CORTA. El clon y los recortes van en tiempo de secuencia, así que en
   * teoría no dependen de ella, pero no está medido: ningún verbo la cambia y el proyecto de prueba
   * no tiene un clip así. La reinserción de antes la ignoraba —la cola salía a 1x y con la entrada
   * corrida— sin avisar. Rebotar antes de tocar nada es mejor que un corte que no se probó.
   */
  const velocidad = await velocidadDe(clip);
  let alReves = false;
  try { alReves = !!(await clip.isSpeedReversed()); } catch (e) { alReves = false; }
  if (velocidad !== 1 || alReves) {
    throw new Error(
      `"${encontrado.nombre}" corre a ${velocidad === null ? "una velocidad que no se pudo leer" : velocidad + "x"}` +
      (alReves ? " y al revés" : "") + ": `cortar` no está medido con otra velocidad, y NO se tocó nada. " +
      "Partilo en Premiere con la C."
    );
  }

  /*
   * EL GRUPO: el clip y sus socios del otro tipo, por `sociosDe`, el emparejado de `desactivar`:
   * junta TODOS los streams de un multicanal y, con dos planos del mismo medio en el mismo instante,
   * desempata por el in-point o no toca. Si el clip es de AUDIO, sus socios son su video y los otros
   * streams de ese video. `soloVideo` no parte el audio: sigue entero debajo de las dos partes.
   *
   * Con una duda se parte SOLO el clip pedido, y se dice: el audio queda entero debajo, que no
   * desincroniza nada. Partir el audio de otro plano creyendo que es el de este sí.
   */
  const soloVideo = params.soloVideo === true;
  let socios = [];
  let duda = null;
  if (!soloVideo) {
    if (!encontrado.esAudio) {
      const s = (await sociosDe(sequence, [clip], true))[0];
      socios = s.socios;
      duda = s.motivo;
    } else {
      const v = (await sociosDe(sequence, [clip], false))[0];
      duda = v.motivo;
      if (v.socios.length > 1) {
        duda = `"${encontrado.nombre}" empareja con ${v.socios.length} planos de video en el mismo instante`;
      } else if (v.socios.length === 1) {
        const a = (await sociosDe(sequence, [v.socios[0].clip], true))[0];
        duda = a.motivo;
        const otros = a.socios.filter((x) => x.pista !== encontrado.pista || x.indice !== encontrado.indice);
        if (!duda) socios = [v.socios[0]].concat(otros);
      }
    }
  }
  const miembros = [{ item: clip, pista: encontrado.pista, nombre: encontrado.nombre }]
    .concat(socios.map((s) => ({ item: s.clip, pista: s.pista, nombre: null })));
  const pistaDe = async (etq) => {
    const m = /^([VA])(\d+)$/.exec(etq);
    const i = Number(m[2]) - 1;
    return m[1] === "V" ? sequence.getVideoTrack(i) : sequence.getAudioTrack(i);
  };
  const deClips = async (etq) => {
    const tr = await pistaDe(etq);
    return tr ? await tr.getTrackItems(ppro.Constants.TrackItemType.CLIP, false) : [];
  };

  /*
   * DÓNDE SE ESTACIONAN LOS CLONES: un segundo después del final, en cuadro. Ahí no hay nada en
   * ninguna pista, y un clon es un overwrite: estacionado ahí no pisa nada. Se toma el mayor entre
   * el final de la secuencia y el del último clip de cada pista del grupo, por si `getEndTime`
   * contara solo una parte. Los clips de una pista vienen en orden, así que el último es el último.
   */
  let tpf = null;
  try { tpf = Number(await sequence.getTimebase()); } catch (e) { tpf = null; }
  const paso = isFinite(tpf) && tpf > 0 ? tpf : TICKS_POR_SEGUNDO;
  let finTicks = Number((await sequence.getEndTime()).ticks);
  for (const m of miembros) {
    const its = await deClips(m.pista);
    const ultimo = its.length ? its[its.length - 1] : null;
    if (ultimo) finTicks = Math.max(finTicks, Number((await ultimo.getEndTime()).ticks));
  }
  const parqueTicks = Math.ceil((finTicks + TICKS_POR_SEGUNDO) / paso) * paso;
  const inicioTicks = Number((await clip.getStartTime()).ticks);
  const finClipTicks = Number((await clip.getEndTime()).ticks);
  const corteTicks = Number(tickCorte.ticks);
  const tk = (n) => ppro.TickTime.createWithTicks(String(Math.round(n)));
  const seg = (n) => Number((n / TICKS_POR_SEGUNDO).toFixed(3));
  // Medio milisegundo: la API guarda el tick exacto; esto es solo para no comparar floats a ciegas.
  const TOL = TICKS_POR_SEGUNDO / 2000;
  const cerca = (a, b) => Math.abs(a - b) <= TOL;

  const itemsAntes = (await contarItems(sequence)).total;
  const trPrincipal = await pistaDe(encontrado.pista);
  const contarTransiciones = async () => {
    try { return (await trPrincipal.getTrackItems(ppro.Constants.TrackItemType.TRANSITION, false)).length; }
    catch (e) { return null; }
  };
  const transicionesAntes = await contarTransiciones();

  /*
   * Cada paso es una transacción aparte y se RELEE antes del siguiente: un paso que no entró no
   * deja al siguiente trabajar sobre un estado que no existe. Si algo falla, se dice qué quedó
   * dónde y cuántos Cmd+Z lo deshacen, que son las transacciones que corrieron. De a
   * `TOPE_LOTE`, y espaciadas: `enLotes` espacia sus lotes y `esperarEntreTx` los pasos.
   */
  let transacciones = 0;
  const correr = async (fabricas, nombre) => {
    const r = await enLotes(project, fabricas.map((f) => [f]), nombre);
    transacciones += r.corrieron;
    return r;
  };
  const deshacer = () => (transacciones ? ` · ${transacciones} Cmd+Z lo deshace${transacciones > 1 ? "n" : ""}` : " · no corrió ninguna transacción");
  const fallar = (que, estado) => {
    const e = new Error(`NO SE PARTIÓ "${encontrado.nombre}" en ${encontrado.pista}: ${que}` +
      (estado ? ` · ${estado}` : "") + deshacer());
    e.quedoAMedias = transacciones > 0;
    throw e;
  };
  ...
  const r1 = await correr(miembros.map((m) => () => ed.createCloneTrackItemAction(m.item, tk(offTicks), 0, 0)),
    "cortar: clonar la cola");
  const clones = [];
  for (const m of miembros) {
    const its = await deClips(m.pista);
    const ultimo = its.length ? its[its.length - 1] : null;
    const aca = ultimo && cerca(Number((await ultimo.getStartTime()).ticks), parqueTicks) ? ultimo : null;
    clones.push(aca);
  }
  const faltan = miembros.filter((m, i) => !clones[i]).map((m) => m.pista);
  if (faltan.length) {
    fallar(`no apareció el clon en ${faltan.join(", ")}` + (r1.errores.length ? ` (${r1.errores.join(" | ")})` : ""),
      estacionados(clones.filter(Boolean).length));
  }

  // 2) Recortarles el inicio hasta el punto de corte, todavía estacionados.
  await esperarEntreTx();
  const inicioCola = parqueTicks + (corteTicks - inicioTicks);
  const r2 = await correr(clones.map((c) => () => c.createSetStartAction(tk(inicioCola))), "cortar: recortar la cola");
  const iniciosCola = [];
  for (const c of clones) iniciosCola.push(Number((await c.getStartTime()).ticks));
  const malRecortados = miembros.filter((m, i) => !cerca(iniciosCola[i], inicioCola)).map((m) => m.pista);
  if (malRecortados.length) {
    fallar(`el inicio de la cola no quedó en el corte en ${malRecortados.join(", ")}` +
      (r2.errores.length ? ` (${r2.errores.join(" | ")})` : ""), estacionados(clones.length));
  }

  // 3) Recortar el original —y sus socios— hasta el punto de corte: la cabeza.
  await esperarEntreTx();
  const r3 = await correr(miembros.map((m) => () => m.item.createSetEndAction(tickCorte)), "cortar: recortar la cabeza");
  const malCabeza = [];
  for (const m of miembros) {
    if (!cerca(Number((await m.item.getEndTime()).ticks), corteTicks)) malCabeza.push(m.pista);
  }
  if (malCabeza.length) {
    fallar(`la cabeza no quedó cortada en ${malCabeza.join(", ")}` + (r3.errores.length ? ` (${r3.errores.join(" | ")})` : ""),
   ...
   * sigue el material, el nombre, y el conteo de TODA la secuencia contra lo esperado —un item más
   * por miembro del grupo—. Ese conteo es justo lo que delató el reporte: 27 → 34 donde tocaba 28.
   */
  const cabeza = await tiemposDe(clip);
  const cola = colas[0];
  ...
  const partes = [
    { desde: cabeza.desde, hasta: cabeza.hasta, entrada: cabeza.entrada },
    { desde: cola.desde, hasta: cola.hasta, entrada: cola.entrada }
  ];
  const deAudio = miembros.filter((m) => /^A/.test(m.pista)).map((m) => m.pista);

  return {
    resumen:
      `"${encontrado.nombre}" en ${encontrado.pista}: ${antes.desde}-${antes.hasta}s → ` +
      partes.map((c) => `${c.desde}-${c.hasta}s (entrada ${c.entrada})`).join(" + ") +
      (pegados ? "" : " · OJO: quedó un hueco entre las partes") +
      ...
      (soloVideo
        ...
        : duda
          ? ` · OJO: ${duda}. Se partió SOLO este clip: el audio sigue entero debajo`
          : miembros.length > 1
            ? ` · partido con ${miembros.length - 1} socio(s) (${miembros.slice(1).map((m) => m.pista).join(", ")}), en sus mismas pistas`
            : encontrado.esAudio ? " · sin video vinculado: se partió solo este clip" : " · sin audio vinculado: la cola tampoco lleva") +
      ` · items en la secuencia ${itemsAntes} → ${itemsDespues}` +
      (itemsDespues === itemsEsperados ? "" : ` · OJO: se esperaban ${itemsEsperados}`) +
      (transicionesAntes !== null && transicionesDespues !== null && transicionesAntes !== transicionesDespues
        ? ` · OJO: ${encontrado.pista} tenía ${transicionesAntes} transición(es) y quedaron ${transicionesDespues}` : "") +
      (miembros.length > 1
        ? ` · la cola queda SIN VÍNCULO en Premiere: la API no crea vínculos. El bridge la sigue emparejando por medio y rango; a mano, Cmd+L`
        : "") +
      ` · ${transacciones} Cmd+Z lo deshace${transacciones > 1 ? "n" : ""}` +
      /* `cuantizado` dice CUANTO se movio el corte para caer en cuadro. Hasta el
         2026-09-19 esta linea decia `dichos`, que es una variable de `editarMarcador`
         y aca no existe: el verbo cortaba, reponia el Motion y limpiaba los in/out, y
         recien entonces tiraba ReferenceError armando el mensaje. Quien lo llamaba veia
         un error y concluia que no habia pasado nada — un FALSO NEGATIVO sobre un corte
         que si ocurrio, que es lo que hace que alguien reintente y corte dos veces.
         Y de paso el dato no llegaba al resumen, que es lo mismo que no tenerlo. */
    ...
    cuantizado: cuantizado,
    socios: miembros.slice(1).map((m) => m.pista),
    audioPartido: deAudio,
    duda: duda,
    sinVinculo: miembros.length > 1,
...
async function sacarRangos(params) {
  ...
  for (let i = 0; i < rangos.length; i++) {
    ...
    try {
      ...
      /* Un corte que quedó A MEDIAS —clones estacionados pasado el final, o una cabeza cortada sin
         su cola— no se tapa con los rangos que siguen: cada operación de más aleja el Cmd+Z que lo
         deshace. Se frena acá, con el estado dicho en el error. */
      if (e && e.quedoAMedias) { frenado = true; break; }
    }
  }
...
async function ponerAjustes(project, sequence, params) {
  ...
  if (fps !== null) {
    ...
    if (antes.fps !== null && Math.abs(antes.fps - fps) < 0.01) {
      ...
      const formas = [
        /*
         * ESTA es la que entra, y estuvo medida en CLAUDE.md antes de estar acá.
         *
         * El verbo enumeraba tres formas y RECHAZABA con las tres —que es el comportamiento
         * correcto: releía y se negaba a devolver una secuencia a 50fps diciendo que salió— pero
         * las tres eran justamente las que NO andan. La que faltaba es el objeto `FrameRate`, y
         * salió de reflejar la clase en vez de deducirla del getter: `FrameRate.createWithValue`.
         * Verificado el 2026-09-11 con CONTROL: la secuencia tocada pasó de 29,98 a 25fps y otra
         * del mismo proyecto, sin tocar, siguió en 29,96.
         *
         * Va PRIMERA para no reintentar a ciegas en cada llamada, igual que `MediaType.ANY` en
         * `borrar`; las otras quedan de red por si cambia en otra versión.
         */
...
async function escalaFija(params) {
  ...
  return {
    ...
    /* `siguiente` es el dato que hace que la continuacion no haya que calcularla: null cuando
     * no quedo nada. Mismo contrato que `leerEscalas`. */
...
async function fijar(params) {
   ...
   */
  const nivel = esNivelDeAudio(efecto, cual);
  if (params.db !== undefined && !nivel) {
    throw new Error(`\`db\` es solo para Volume > Level, el único param con la escala medida; "${efecto} > ${cual}" ` +
      "se escribe con `valor`. NO se escribió nada.");
  }
  const pedido = nivel ? nivelPedido(params, `${efecto} > ${cual}`) : params.valor;
  let valor;
  const esTexto = typeof pedido === "string";
  if (typeof params.x === "number" && typeof params.y === "number") valor = new ppro.PointF(params.x, params.y);
  else if (typeof pedido === "number" || typeof pedido === "boolean" || esTexto) valor = pedido;
  ...
  const leido = cual === "Position" ? aPunto(despues)
    ...
    : aNumero(despues);
  const esperado = cual === "Position" ? { x: params.x, y: params.y } : pedido;
  /* La tolerancia es RELATIVA. Era 0,5 absoluto, y en Level —que va de 0 a 1— eso daba por quedado
     cualquier cosa: pedir 0,02 y que siguiera en 0,1778 se informaba bien. En Level se compara en dB. */
  const quedo = cual === "Position"
    ? leido && Math.abs(leido.x - params.x) < 0.002 && Math.abs(leido.y - params.y) < 0.002
    : esTexto ? String(leido) === String(pedido)
    : leido === null ? false
    : nivel ? (pedido === 0 ? leido === 0 : Math.abs(crudoADb(leido) - crudoADb(pedido)) < 0.05)
    : Math.abs(leido - pedido) <= Math.max(1e-4, Math.abs(pedido) * 1e-3);
  const conDb = (x) => (nivel && typeof x === "number" ? `${JSON.stringify(x)} (${textoDb(x)})` : JSON.stringify(x));
...
async function unirAudio(params) {
  ...
  let unidos = 0, revisados = 0, seleccionSucia = 0;
  let frenado = null;
  const fallidos = [];

  for (let t = 0; t < naudio && frenado === null; t++) {
    let vueltas = 0;
    ...
    while (vueltas++ < 200) {
      ...
      if (par < 0) break;
      const cupo = borradoPermitido();
      if (!cupo.ok) { frenado = cupo.libreEn; break; }
      revisados++;
  ...
  return {
    resumen:
      ...
      (frenado !== null
        ? ` · SE FRENÓ en el tope de borrados (${BORRAR_TOPE} en ${BORRAR_VENTANA_MS / 1000}s, el de \`borrar\`): ` +
          `puede quedar algo por unir; volvé a llamar en ${frenado}s`
        : "") +
      (seleccionSucia ? ` · OJO: ${seleccionSucia} vez/veces la selección NO quedó vacía: deseleccioná en Premiere antes de otra edición` : "") +
      (fallidos.length ? ` · FALLARON: ${fallidos.slice(0, 3).join(" | ")}` : ""),
    unidos: unidos, quedan: quedan, fallidos: fallidos, frenadoPorElTope: frenado !== null
  };
}
    ...
    try {
      ...
      if (motion) {
        ...
        if (pEsc) {
           ...
           * (baja la ráfaga, no saca el puntero), por qué otro plugin no crashea (lo
...
async function unirVideo(params) {
  ...
  const hechos = [], fallidos = [];
  let frenado = null, seleccionSucia = 0;
  ...
  return {
    resumen:
      ...
      (frenado !== null
        ? ` · SE FRENÓ en el tope de borrados (${BORRAR_TOPE} en ${BORRAR_VENTANA_MS / 1000}s, el de \`borrar\`): ` +
          `los rangos que seguían NO se tocaron; volvé a llamar en ${frenado}s`
        : "") +
      (seleccionSucia ? ` · OJO: ${seleccionSucia} vez/veces la selección NO quedó vacía: deseleccioná en Premiere antes de otra edición` : "") +
      (fallidos.length ? ` · FALLARON: ${fallidos.slice(0, 3).join(" | ")}` : ""),
    hechos: hechos.length, quedan: total, fallidos: fallidos, frenadoPorElTope: frenado !== null
  };
}
...
async function marcadores(params) {
  ...
  for (let i = 0; i < lista.length; i++) {
     ...
     * CLAUDE.md sin estado que leer. Los 26 marcadores de sección de un evento hubo que
...
async function marcar(params) {
  ...
  /*
   * SOBRE LA SECUENCIA O SOBRE UN CLIP, con el mismo direccionamiento que `desmarcar`.
   *
   * Faltaba, y era una asimetria: `desmarcar` sabia sacar los marcadores de un clip y este verbo
   * no sabia ponerlos. Por que importa: un marcador de SECUENCIA describe un instante del
   * TIMELINE y uno de clip un instante del MATERIAL. Para una grilla de beats sobre una pista de
   * musica el correcto es el del clip — si el editor corre el tema medio segundo, los 180
   * marcadores de secuencia quedan todos mal y hay que rehacerlos.
   *
   * Y con `clip` el `segundos` es TIEMPO DE FUENTE, no de timeline. Se dice en el resumen porque
   * pasar un tiempo de timeline pondria el marcador en otro lado SIN error.
   */
  const objetivo = await sujetoDeMarcadores(sequence, params);
  ...
  return {
    resumen:
      ...
      /*
       * Los dos avisos del marcador de clip son consecuencias que NO se ven y que cambian lo
       * que uno tiene que pasar: el tiempo es de FUENTE —pasar uno de timeline lo pone en otro
       * lado sin error— y el marcador vive en el MEDIO, asi que aparece en toda instancia.
       */
      (objetivo.esClip
...
/**
 * Cuantiza un tiempo al frame de la secuencia, y dice CUANTO movio.
 *
 * En la interfaz de Premiere no se puede poner un marcador entre frames, asi que subframe no es
 * un estado que el usuario pueda crear a mano: lo producian estos verbos. Se descubrio el
 * 2026-09-03 con 139 marcadores en el beat de una musica a 96 BPM —el beat mide 15,625 cuadros a
 * 25fps, asi que 121 de los 139 quedaron subframe— y lo vio el editor, no ninguna verificacion.
 *
 * Estaba adentro de `marcar`. Se extrajo cuando aparecio `editarMarcador`, porque
 * `createMoveMarkerAction` TAMPOCO cuantiza —medido: pedir 9,017s a 25fps deja 9,02, medio
 * cuadro— y dejarlo suelto habria reintroducido el mismo bug en el verbo de al lado. Una regla
 * implementada en un verbo no se aplica sola al de al lado: es el estribillo de este repo.
 *
 * La via es aritmetica entera sobre ticks con `getTimebase()`, que son ticks por frame;
 * `alignToNearestFrame` contesta "Illegal Parameter type", ya medido en `cortar`.
 */
...
/**
 * Cambia un marcador YA PUESTO: lo mueve, le cambia el color o el rango.
 *
 * Hasta ahora corregir un marcador mal puesto era BORRARLO Y REHACERLO — con su nombre, su
 * comentario y su color, que hay que volver a pasar—. Los tres metodos existian y estaban
 * medidos en la sonda: `createMoveMarkerAction`, `createSetColorByIndexAction` y
 * `createSetDurationAction`.
 *
 * **Y el movimiento se CUANTIZA.** `createMoveMarkerAction` no lo hace: medido, pedir 9,017s a
 * 25fps deja el marcador en 9,02 — medio cuadro. Exponerlo crudo seria reintroducir el bug que
 * `marcar` ya pago con 121 marcadores de 139 subframe, en el verbo de al lado.
 *
 * El veredicto sale de RELEER la lista, nunca del booleano de la transaccion.
 */
...
/**
 * A QUIEN se le ponen o quitan los marcadores: la secuencia, o un CLIP.
 *
 * Estaba escrito adentro de `desmarcar` y ahora lo comparten los tres verbos, porque dejarlo en
 * uno solo era justamente el pendiente: `desmarcar` sabia direccionar un clip, `marcar` no, y
 * `marcadores` tampoco — asi que un marcador de clip se podia borrar pero no crear ni leer.
 *
 * EL SUJETO ES EL MEDIO, no el TrackItem. `Markers.getMarkers(trackItem)` contesta "Invalid
 * parameter." —tipos bien, valor mal— y los marcadores que crea la deteccion de escenas viven en
 * el ClipProjectItem. Consecuencia que hay que saber al usarlo: **son del MATERIAL, asi que se ven
 * en toda instancia de ese medio**, no solo en el clip que se apunto. Para una grilla de beats
 * sobre una pista de musica eso es exactamente lo que se quiere —sobreviven a mover el audio— y
 * para otras cosas puede sorprender, por eso los verbos lo dicen en el resumen.
 *
 * Se prueban los dos candidatos y se informa cual contesto: "marcador de clip" en esta API puede
 * significar cualquiera de los dos y no esta documentado.
 */
...
async function revisar(params) {
  ...
  for (const g of grupos) {
    for (let t = 0; t < g.cuantas; t++) {
       ...
       * Se lee despues del `continue` a proposito: una pista vacia y apagada no es noticia.
       */
      const apagada = await estadoDeSalida(track);
  ...
  const partes = [];
  /*
   * Va PRIMERO, antes que los ceros y los solapes, porque una pista sin salida INVALIDA
   * todo lo demas que este verbo pueda informar: los clips pueden estar perfectos y no
   * verse. Y porque mientras esto no estuviera en `partes`, `revisar` imprimia
...
async function leerParam(params) {
  ...
  for (let i = desdeI; i < hastaI; i++) {
    ...
    try {
      ...
      if (esNivelDeAudio(efecto, fila.param) && typeof fila.valor === "number") fila.db = Number(crudoADb(Number(num)).toFixed(2));
    } catch (e) { fila.error = e && e.message ? e.message : String(e); }
    salida.push(fila);
  }
  ...
  return {
    resumen: `${etiqueta} · ${efecto} > ${nombreParam || "#" + iParam} · leídos ${salida.length} de ${items.length}` +
      ...
      ` · ` + salida.filter((f) => f.valor !== null).map((f) => `[${f.indice}]=${JSON.stringify(f.valor)}` +
...
async function radiografia(params) {
  ...
  for (let i = desdeI; i < hastaI; i++) {
    ...
    /* El comentario viejo decia "los de audio no lo tienen" y es FALSO: medido el
     * 2026-09-17, un clip de audio contesta `isDisabled()` igual que uno de video. */
    fila.desactivado = await ojito(it);
...
async function colocarLote(params) {
  ...
  /* La pista tiene que EXISTIR antes de escribir, como en `insertar`: la verificación de abajo corre
     DESPUÉS de las transacciones, y con una pista de más `getVideoTrack` tira en vez de devolver null,
     así que lo que Premiere hubiera hecho quedaba hecho y el verbo contestaba la excepción cruda. */
  const totalV = await sequence.getVideoTrackCount();
  if (pistaIndex >= totalV) {
    throw new Error(`"${sequence.name}" tiene ${totalV} pista(s) de video (V1 a V${totalV}); se pidió ${pista}. NO se ejecutó nada.`);
  }
...
/*
 * SONDA de `Transcript.transcribeClipProjectItem`, que aparecio en 26.5 (2026-09-11).
 *
 * Este archivo documenta —con una investigacion de 16 formas probadas detras— que CREAR
 * una transcripcion por API no se podia. En 26.5 el metodo existe y hay que medir QUE
 * hace, no asumirlo: `api` lee nombres, no llama.
 *
 * Por defecto SOLO LEE: que idiomas dice soportar y si el pack esta disponible. La
 * transcripcion de verdad pide `correr: true`, porque baja un modelo, tarda, y no se
 * dispara sin querer. La firma reporta aridad 2 y no esta documentada, asi que se
 * ENUMERAN formas y se informa cual entro — el patron de `exportar` y `marcar`.
 */
async function sondaTranscribir(params) {
  const project = await getProyecto();
  if (!ppro.Transcript || typeof ppro.Transcript.transcribeClipProjectItem !== "function") {
    throw new Error("Esta version de Premiere no expone `Transcript.transcribeClipProjectItem`. " +
      "Aparecio en 26.5; `api` sobre `Transcript` dice cuantos metodos hay.");
  }
  const info = { idiomas: null, packDisponible: null, yaTenia: null, via: null, resultado: null };

  try {
    const l = await ppro.Transcript.querySupportedLanguages();
    info.idiomas = Array.isArray(l) ? l : (l && l.length !== undefined ? Array.from(l) : String(l));
  } catch (e) { info.idiomas = "querySupportedLanguages tiro: " + (e && e.message ? e.message : e); }

  const idioma = typeof params.idioma === "string" ? params.idioma : "es-ES";
  try {
    info.packDisponible = await ppro.Transcript.isLanguagePackAvailable(idioma);
  } catch (e) { info.packDisponible = "tiro: " + (e && e.message ? e.message : e); }

  let clipItem = null, nombre = null;
  if (params.medio) {
    const m = await buscarMedio(project, params.medio);
    nombre = String(m.name);
    try { clipItem = ppro.ClipProjectItem.cast(m); } catch (e) { clipItem = null; }
    if (!clipItem) throw new Error(`"${nombre}" no se pudo castear a ClipProjectItem.`);
    try { info.yaTenia = await ppro.Transcript.hasTranscript(clipItem); } catch (e) { info.yaTenia = null; }
  }

  if (params.correr === true && clipItem) {
    /* Aridad 2, sin documentar. Se prueban las formas plausibles y se informa cual entro. */
    const formas = [
      ["(clipItem, idioma)", () => ppro.Transcript.transcribeClipProjectItem(clipItem, idioma)],
      ["(clipItem, {language})", () => ppro.Transcript.transcribeClipProjectItem(clipItem, { language: idioma })],
      ["(clipItem, {languageCode})", () => ppro.Transcript.transcribeClipProjectItem(clipItem, { languageCode: idioma })],
      ["(clipItem, null)", () => ppro.Transcript.transcribeClipProjectItem(clipItem, null)],
    ];
    for (const [etq, fn] of formas) {
      try {
        const r = await fn();
        info.via = etq;
        info.resultado = (r && typeof r === "object") ? Object.keys(r).slice(0, 12) : String(r);
        break;
      } catch (e) { info["forma " + etq] = String((e && e.message) || e).slice(0, 130); }
    }
    /* EL VEREDICTO SALE DEL ESTADO: que la llamada no tire no dice que haya transcrito. */
    try { info.tieneDespues = await ppro.Transcript.hasTranscript(clipItem); } catch (e) { info.tieneDespues = null; }
  }

  return {
    resumen:
      `Transcript en esta versión: transcribeClipProjectItem EXISTE` +
      (nombre ? ` · medio "${nombre}"` : " · sin medio, sólo se consultó la API") +
      (info.yaTenia !== null ? ` · tenía transcripción: ${info.yaTenia}` : "") +
      ` · pack "${idioma}": ${info.packDisponible}` +
      (params.correr === true
        ? (info.via ? ` · CORRIÓ vía ${info.via} · tiene transcripción ahora: ${info.tieneDespues}`
                    : " · NINGUNA forma entró (ver detalle)")
        : " · NO se transcribió (pedilo con `correr: true`)"),
    info: info,
  };
}
...
/*
 * SONDA de las TRANSICIONES, que este archivo daba por no expuestas (2026-09-11).
 *
 * El flujo aparecio reflejando, y la pieza que faltaba estaba en el CLIP, no en la pista:
 * `VideoClipTrackItem.createAddVideoTransitionAction`. La firma no esta documentada —todas
 * reportan aridad 0, que ya se sabe que miente— asi que se ENUMERAN formas y se informa
 * cual entro, como con `exportar` y `marcar`.
 *
 * EL VEREDICTO SALE DE RELEER LA PISTA con `getTrackItems(TRANSITION)`, no de que la
 * transaccion devuelva true.
 */
/*
 * TRANSICION de video en un CORTE. Todo lo de aca esta MEDIDO en el proyecto de prueba el
 * 2026-09-19, con controles; nada se dedujo del nombre de un metodo.
 *
 *   setApplyToStart(bool)       cabeza (true) o cola (false) del clip que se pasa.
 *                               Medido con el control: sobre el mismo clip 5–10,04s,
 *                               true la puso en el corte de 5,00 y false en el de 10,04.
 *   setTransitionAlignment(f)   una FRACCION 0..1, NO los `Constants.TransitionPosition`:
 *                               0 arranca en el corte · 0,5 centrada · 1 termina en el corte
 *                               · mayor a 1 se clampea. Medido con cuatro valores.
 *   setDuration(tick)           el largo.
 *
 * LA TRAMPA: `Constants.TransitionPosition.START` vale 1, y pasarlo da "TERMINA en el
 * corte", que es el lado contrario al que sugiere el nombre. Por eso este verbo toma la
 * fraccion y no el enum.
 *
 * TODO va adentro de UN `lockedAccess` y sin un solo `await` en el medio: creado afuera, el
 * objeto de transicion se invalida y tira "The script object is no longer valid". Es la misma
 * leccion que ya costo `createBinAction`.
 *
 * EL VEREDICTO SALE DE CONTAR, no de que la transaccion devuelva true: releer una transicion
 * devuelve `null` y ni `queryCast` ni `castOrThrow` la recuperan, asi que lo unico que la API
 * da es `getTrackItems(TRANSITION).length`. Para ver DONDE quedo hay que abrir el `.prproj`.
 *
 * Y LO QUE ESTE VERBO NO HACE, que se dice en cada respuesta porque no se ve: el AUDIO.
 * `TransitionFactory` solo expone `createVideoTransition`, y el clip de audio no tiene ningun
...
async function transicion(params) {
  ...
  const pista = typeof params.pista === "string" ? params.pista : "V1";
  const idx = Number(String(pista).replace(/[^0-9]/g, "")) - 1;
  if (!(idx >= 0)) throw new Error(`Pista "${pista}" no entendida; se espera "V1", "V2"…`);
  const track = await sequence.getVideoTrack(idx);
  if (!track) throw new Error(`No existe la pista ${pista} en "${await nombreDeSecuencia(sequence)}".`);

  const cuantas = async () => {
    const t = await track.getTrackItems(ppro.Constants.TrackItemType.TRANSITION, false);
    return t ? t.length : 0;
  };
  const clips = (await track.getTrackItems(ppro.Constants.TrackItemType.CLIP, false)) || [];
  if (!clips.length) throw new Error(`${pista} no tiene clips.`);

  const bordes = [];
  for (let k = 0; k < clips.length; k++) {
    const leer = async (fn) => { try { const v = await fn(); return (v && typeof v.seconds === "number") ? v.seconds : null; } catch (e) { return null; } };
    bordes.push({ i: k,
      desde: await leer(() => clips[k].getStartTime && clips[k].getStartTime()),
      hasta: await leer(() => clips[k].getEndTime && clips[k].getEndTime()) });
  }

  /*
   * Direccionar por TIEMPO es lo natural —un editor piensa "un fundido a los 12,5s", no
   * "la cabeza del clip 3"— asi que `segundos` resuelve a clip+lado y se INFORMA a cual
   * cayo. La forma explicita (`indice` + `alInicio`) queda para quien ya sabe cual quiere.
   */
  let clipIdx = null, alInicio = null, corteEn = null;
  if (typeof params.segundos === "number") {
    let mejor = null;
    for (const b of bordes) {
      for (const [t, i, ini] of [[b.desde, b.i, true], [b.hasta, b.i, false]]) {
        if (t === null || t <= 0.0001) continue;                 // el arranque de la pista no es un corte
        if (i === bordes.length - 1 && ini === false) continue;   // ni el final
        const d = Math.abs(t - params.segundos);
        if (!mejor || d < mejor.d) mejor = { d: d, t: t, i: i, ini: ini };
      }
    }
    if (!mejor) throw new Error(`${pista} no tiene ningun corte al que apuntar.`);
    if (mejor.d > 1.0) {
      throw new Error(
        `El corte mas cercano a ${params.segundos}s en ${pista} esta en ${mejor.t.toFixed(2)}s, ` +
        `a ${mejor.d.toFixed(2)}s. Se rechaza por las dudas: poner un fundido en el corte equivocado ` +
        `no tira error y solo se ve reproduciendo. Los cortes son: ` +
        bordes.slice(1).map((b) => b.desde.toFixed(2)).join(", ") + ".");
    }
    clipIdx = mejor.i; alInicio = mejor.ini; corteEn = mejor.t;
  } else if (typeof params.indice === "number") {
    clipIdx = params.indice;
    alInicio = params.alInicio !== false;
    const b = bordes[clipIdx];
    if (!b) throw new Error(`No hay clip ${clipIdx} en ${pista} (hay ${bordes.length}).`);
    corteEn = alInicio ? b.desde : b.hasta;
  } else {
    throw new Error("Falta decir DONDE: `segundos` (el corte) o `indice` + `alInicio`.");
  }
  const clip = clips[clipIdx];
  if (!clip) throw new Error(`No hay clip ${clipIdx} en ${pista}.`);

  /* El largo va en la grilla de cuadro, como `marcar` y `cortar`: un fundido de 1,013s
     termina entre cuadros, que es la mitad del mismo defecto. */
  const pedida = typeof params.duracion === "number" ? params.duracion : 1;
  const q = await cuantizarAlFrame(sequence, aTick(pedida));
  const dur = q.tick;

  let frac = typeof params.alineacion === "number" ? params.alineacion : 0.5;
  let avisoFrac = "";
  if (frac < 0 || frac > 1) {
    avisoFrac = ` · OJO: alineacion ${frac} fuera de 0..1, la API la CLAMPEA (medido: 2 da lo mismo que 1)`;
    frac = Math.max(0, Math.min(1, frac));
  }

  /*
   * EL HANDLE: cuanto material hay MAS ALLA del corte, que es lo que decide si el fundido
   * pedido entra o Premiere lo acorta EN SILENCIO.
   *
   * Reportado por una sesion real el 2026-09-21: el verbo informo "Cross Dissolve de 1.00s,
   * centrada en el corte" y la transicion salio de 0,5s, medido en el timeline. El verbo no
   * puede releerla —los items de `getTrackItems(TRANSITION)` son null, remedido hoy con las
   * tres formas— pero SI puede calcular lo que hay disponible y avisar antes.
   *
   * Un fundido necesita material de los DOS lados del corte:
   *   el clip que SALE tiene que seguir reproduciendose  ->  mediaDur - suOutPoint
   *   el que ENTRA tiene que empezar antes               ->  suInPoint - mediaStart
   *
   * Y cuanto necesita de cada lado depende de la alineacion: con 0,5 (centrada) es la mitad
   * de cada uno; con 0 todo del que sale; con 1 todo del que entra.
   *
   * La duracion del medio salio de una sonda de hoy y no se podia deducir: `ClipProjectItem`
   * NO expone ninguna duracion —reflejados sus 40 metodos— y el camino es `getMedia()`, cuyo
   * objeto si tiene `getDuration()`. Ojo con la trampa de siempre: la PROPIEDAD `duration`
   * devuelve una Promise y el GETTER devuelve el valor.
   *
   * Si no se puede leer, se DICE que no se pudo. Un handle que no se midio no es un handle
   * que alcance, y callarlo reintroduce el defecto que esto viene a cerrar.
   */
  const handleDe = async (c) => {
    if (!c) return null;
    try {
      const inP = aSegundos(await c.getInPoint());
      const outP = aSegundos(await c.getOutPoint());
      const pi = await c.getProjectItem();
      const ci = ppro.ClipProjectItem.cast(pi);
      const media = await ci.getMedia();
      const dur = (await media.getDuration()).seconds;
      const ini = (await media.getStart()).seconds;
      return { entrada: inP, salida: outP, medioDura: dur, medioEmpieza: ini,
               antes: inP - ini, despues: (ini + dur) - outP };
    } catch (e) { return { error: String((e && e.message) || e).slice(0, 90) }; }
  };
  /* Una transicion es la COLA de un clip y la CABEZA del siguiente. Cual es cual sale de
     `alInicio`, que ya esta resuelto arriba por tiempo o explicito. */
  const hSale  = await handleDe(alInicio ? clips[clipIdx - 1] : clips[clipIdx]);
  const hEntra = await handleDe(alInicio ? clips[clipIdx] : clips[clipIdx + 1]);

  const pedidaSeg = aSegundos(dur);
  let avisoHandle = "";
  {
    const ok = (h) => h && !h.error && Number.isFinite(h.antes) && Number.isFinite(h.despues);
    const necesitaAntes = pedidaSeg * frac;          // del que ENTRA, antes de su in-point
    const necesitaDespues = pedidaSeg * (1 - frac);  // del que SALE, despues de su out-point
    const topes = [];
    if (ok(hEntra) && frac > 0.001) topes.push({ lado: "el clip que ENTRA", hay: hEntra.antes, necesita: necesitaAntes, max: hEntra.antes / frac });
    if (ok(hSale) && frac < 0.999) topes.push({ lado: "el clip que SALE", hay: hSale.despues, necesita: necesitaDespues, max: hSale.despues / (1 - frac) });
    const cortos = topes.filter((x) => x.hay < x.necesita - 0.001);
    if (cortos.length) {
      const maxPosible = Math.min.apply(null, topes.map((x) => x.max));
      avisoHandle =
        ` · OJO: PEDISTE ${pedidaSeg.toFixed(3)}s Y EL HANDLE NO DA. ` +
        cortos.map((x) => `${x.lado} tiene ${x.hay.toFixed(3)}s y necesita ${x.necesita.toFixed(3)}s`).join("; ") +
        `. Premiere la va a ACORTAR a ~${Math.max(0, maxPosible).toFixed(3)}s sin avisar, y no se puede releer para comprobarlo`;
    } else if (!topes.length) {
      avisoHandle = " · el handle NO se pudo medir" +
        (hSale && hSale.error ? ` (sale: ${hSale.error})` : "") +
  ...
  /*
   * El default es el Cross Dissolve MODERNO, que es el que Premiere pone con Cmd+D.
   * `ADBE Cross Dissolve` NO es ese: entra como "Cross Dissolve (Legacy)" —medido leyendo
   * el DisplayName del `.prproj`, y lo cazo el editor mirando el timeline—. Los dos
   * matchName salieron del XML del proyecto de prueba, que tenia una de cada:
   *     «Cross Dissolve»          -> AE.AE_Impact_Dissolve
   *     «Cross Dissolve (Legacy)» -> ADBE Cross Dissolve
   */
  const nombre = typeof params.matchName === "string" ? params.matchName : "AE.AE_Impact_Dissolve";
  /* Un matchName que no existe hace que `createVideoTransition` no devuelva nada y el verbo
     diria "no se agrego" sin decir por que. Se comprueba ANTES y se ofrece la lista. */
  let conocidos = null;
  try {
    const l = await ppro.TransitionFactory.getVideoTransitionMatchNames();
    conocidos = Array.isArray(l) ? l.map(String) : Array.from(l || []).map(String);
  } catch (e) { conocidos = null; }
  if (conocidos && conocidos.length && conocidos.indexOf(nombre) === -1) {
    const parecidos = conocidos.filter((x) => /dissolve|disolv/i.test(x)).slice(0, 8);
    throw new Error(
      `La transicion "${nombre}" no esta entre las ${conocidos.length} que conoce Premiere. ` +
      (parecidos.length ? `Disolvencias disponibles: ${parecidos.join(", ")}. ` : "") +
      `NO se ejecuto nada.`);
  }
  const antes = await cuantas();

  let ok = false, excepcion = null;
  try {
    project.lockedAccess(() => {
      const t = ppro.TransitionFactory.createVideoTransition(nombre);
      const o = new ppro.AddTransitionOptions();
      o.setDuration(dur);
  ...
  const despues = await cuantas();
  const donde = frac <= 0.05 ? "arranca EN el corte" : frac >= 0.95 ? "TERMINA en el corte"
  ...
  if (despues > antes) {
    return {
      resumen: base + ` · UN Cmd+Z la saca` +
        ` · el AUDIO NO se toca: la API no tiene transiciones de audio, el crossfade va a mano` +
        (q.dicho && /NO SE CUANTIZ/.test(q.dicho) ? ` · ${q.dicho}` : ` · ${q.dicho}`) + avisoFrac +
        /* El aviso del handle va ANTES del "no se puede releer", porque es la unica cosa que
           este verbo SI puede afirmar sobre la duracion real. */
        avisoHandle +
        ` · DONDE quedo no se puede releer por API (devuelve null): esto cuenta que aparecio una, no que este bien puesta`,
      puesta: true, pista: pista, corte: corteEn, clip: clipIdx, alInicio: alInicio,
      alineacion: frac, duracion: aSegundos(dur), antes: antes, despues: despues, transaccion: ok,
      handleSale: hSale, handleEntra: hEntra,
    };
  }
  return {
...
/* Los eventos que llegaron, acumulados ENTRE llamadas. El panel es un contexto largo
 * —poletea sin parar— asi que un array de modulo sobrevive de un comando al siguiente, y
 * eso es lo que hace medible un evento: registrar, provocar, y leer despues. */
const EVENTOS_VISTOS = [];
let EVENTOS_REGISTRADOS = null;

/*
 * SONDA de EventManager (2026-09-11).
 *
 * Lo que interesa no es el mecanismo sino UN evento concreto:
 * `OperationCompleteEvent.EVENT_EXPORT_MEDIA_COMPLETE`. Este archivo dice que el modo `ame`
 * de `exportar` no se puede confirmar "porque el archivo lo escribe otro proceso despues".
 * Si ese evento llega, se puede.
 *
 * Se prueba con IMPORT_MEDIA_COMPLETE, que es barato de provocar y usa el mismo mecanismo.
 */
/*
 * `VideoTrack` expone isMuted/setMute IGUAL que `AudioTrack`, y este repo tiene documentado
 * —con un cuadro negro que se entrego— que "una pista con el OJO APAGADO no la ve NINGUN
 * verbo". Que el metodo EXISTA no dice que sea el ojito: en video el control es Toggle Track
 * Output y el nombre dice "mute", que es vocabulario de audio. Eso se decide MIRANDO EL
 * CUADRO y midiendolo, no por el nombre del metodo ni porque la llamada no tire.
 */
/*
 * Lo que queda de la API, medido por METODO y no por objeto. El inventario de objetos se
 * cerro el 2026-09-11; este es el paso siguiente: `Sequence` tiene 33 metodos de instancia
 * y el bridge usaba 20, y varios de los que faltaban cierran agujeros que este archivo
 * tiene documentados como ABIERTOS —la nest, el relink, los LUT, el texto en pantalla—.
 *
 * Los modos estan separados POR RIESGO a proposito: `leer` es el default y no escribe nada.
 * Lo que escribe se pide, y el veredicto sale de RELEER, nunca de que la llamada no tire.
 */
/*
 * LOS PROYECTOS ABIERTOS, no sólo el que tiene foco (2026-09-11)
 *
 * `guardar` guarda el proyecto CON FOCO, y eso alcanzaba mientras hubiera uno solo. Con dos
 * abiertos, el Cmd+Q del macro saca el cartel de "guardar antes de cerrar" **del otro**, ese
 * cartel BLOQUEA el quit, y `recargar.js --reiniciar` se cuelga los 45s y termina culpando al
 * macro — que es el informe equivocado que este archivo ya documenta tres veces.
 *
 * Se enumeran con `ProjectUtils.getProjectViewIds()` + `getProjectFromViewId()`. Medido: con
 * UN proyecto abierto devuelve 1 y con DOS devuelve 2.
 */
/*
...
async function clonar(params) {
  ...
  /*
   * LA PISTA DESTINO: una que existe, o LA SIGUIENTE a la última, que el clon crea (2026-09-26).
   * `getVideoTrack` con un índice de más no devuelve null: TIRA «BE: An invalid track index was
   * passed to the sequence.», y el verbo moría antes de clonar con ese mensaje crudo (un reporte de uso,
   * 2026-09-25) —y lo que se decía, que clonar CREA pistas, salía de la sonda, nunca de este verbo—.
   * Medido con la sonda el 2026-09-26: un clon de V3 a una V4 que no existía creó UNA pista —3 → 4— y
   * cayó en ella, sin tocar V3. Así que se acepta exactamente la siguiente, y se relee que la cuenta
   * subió en uno y que el clon está ahí. Más arriba no está medido: rebota.
   */
  if (destIdx > cuantasV) {
    throw new Error(
      `No existe V${params.aPista}: "${sequence.name}" tiene ${cuantasV} pista(s) de video (V1 a V${cuantasV}), ` +
      `y NO se ejecutó nada. \`clonar\` crea como mucho la siguiente, V${cuantasV + 1}; más arriba, agregalas en Premiere.`
    );
  }
  const pistaNueva = destIdx === cuantasV;
  ...
  const track = pistaNueva ? null : await sequence.getVideoTrack(destIdx);
  ...
  const trackDespues = destIdx < cuantasDespues ? await sequence.getVideoTrack(destIdx) : null;
  ...
  return {
    resumen:
      ...
      ` · tiempo: ${cuantizado}` +
      (error ? ` · TIRÓ: ${error}` : "") +
      (pistaNueva && cuantasDespues === cuantasV + 1 ? ` · CREÓ V${params.aPista}, la pista pedida` : "") +
      (cuantasDespues !== cuantasV + (pistaNueva && quedo ? 1 : 0)
        ? ` · OJO: la secuencia pasó de ${cuantasV} a ${cuantasDespues} pistas de video` +
          (pistaNueva ? ` y se esperaba ${cuantasV + 1}` : " sin que se pidiera ninguna")
        : "") +
      (cuantasDespues > cuantasV ? " · no hay API para borrar pistas: si sobra, a mano" : "") +
      " · el clon es INDEPENDIENTE: sus efectos se retocan sin tocar el original" +
      " · UN Cmd+Z lo saca" + (pistaNueva ? " (si también saca la pista creada, no está medido)" : ""),
...
const PARAMS_DE = {
  ...
  transicion: ["alInicio", "alineacion", "duracion", "indice", "matchName", "pista", "segundos"],
  sondaTranscribir: ["medio", "idioma", "correr"],
...
async function ejecutar(cmd, params) {
  ...
  if (typeof p.proyecto === "string" && p.proyecto.trim()) {
    ...
    /*
     * UN PROYECTO DE RECUPERACION PASA LA COMPARACION POR NOMBRE (2026-09-11).
     *
     * La comparacion de abajo es por SUBCADENA, y eso es deliberado: pedir "Proyecto" y que
     * enganche "Proyecto - Corte final" es comodo y se usa. Pero Premiere nombra sus
     * recuperaciones como el proyecto MAS un GUID —`Proyecto--a748961a-ff12-…`— asi que
     * CONTIENEN el nombre pedido y la guarda las deja pasar.
     *
     * Medido: despues de que Premiere se cayera, reabrio la recuperacion, y una tanda
     * entera —placements, cinco `guardar`, la relectura— se ejecuto contra ella con la
     * guarda contestando que si. Todo termino escrito en
     * `…/Adobe Premiere Pro Auto-Save/RecoveryProjects/…`. En una copia descartable da
     * igual; en un proyecto real es trabajar una sesion entera creyendo que se guarda
     * donde no se guarda, y el `.prproj` bueno queda igual que al empezar.
     *
     * Se decide por la RUTA y no por el nombre: el GUID podria cambiar de formato, la
     * carpeta es la que Premiere usa. Y va ANTES de la comparacion, porque estar parado
     * en una recuperacion es un problema tenga el nombre que tenga.
     *
     * No hay escape a proposito: si de verdad se quiere trabajar sobre una recuperacion,
     * lo correcto es un "Save As" a una ruta de verdad primero, que ademas es lo que uno
     * quiere que pase.
     */
    if (/Adobe Premiere Pro Auto-Save/i.test(ruta)) {
      throw new Error(
        `"${cmd}": NO se ejecutó nada. El proyecto con foco es un AUTO-SAVE o una RECUPERACIÓN, ` +
        `no el proyecto. Se llama "${nom}" —que contiene "${p.proyecto}", por eso la guarda ` +
        `por nombre no alcanza— y vive en ${ruta}. ` +
        "Todo lo que escribas ahí se guarda en la carpeta de recuperación y tu .prproj queda " +
        "igual que estaba. Abrí el proyecto de verdad, o hacé Save As a una ruta definitiva."
      );
    }
     ...
     *
     * El match parcial es deliberado y se usa —pedir "Proyecto" y que enganche "Proyecto - Corte
     * final"—, asi que sacarlo romperia llamadas correctas. Lo que no es deliberado es que el
  ...
  if (typeof p.secuencia === "string" && p.secuencia.trim()) {
    ...
    /*
     * EL MATCH PARCIAL NO ALCANZA CUANDO HAY OTRA SECUENCIA QUE TAMBIEN LO CONTIENE.
     *
     * La comparación por subcadena es deliberada y se usa --pedir "Proyecto" y que enganche
     * "Proyecto - Corte final"--. Pero un proyecto con las dos versiones de un video
     * tiene "SPOT v3" y "SPOT v3 ENG", y ahí pedir la primera PASA con la segunda
     * activa. Medido el 2026-09-17: se estuvo a punto de colocar 30 subtítulos sobre los
     * 30 que ya tenía la otra secuencia. Lo frenó el colocador al ver la pista ocupada,
     * no esta guarda.
     *
     * Es el mismo agujero que la guarda de `proyecto` ya cierra unas líneas más arriba, y
     * se cierra igual: si el match es PARCIAL y hay más de una secuencia que lo contiene,
     * no hay forma de saber cuál se quiso. Un nombre EXACTO pasa siempre, así que esto no
     * rechaza uso correcto -- que es el peor modo de fallo de una guarda.
     */
    if (nombre.toLowerCase() !== p.secuencia.toLowerCase()) {
      let todas = [];
      try {
        const lista = await project.getSequences();
        for (let i = 0; i < lista.length; i++) {
          try { todas.push(String(lista[i].name)); } catch (e) { /* esa no se pudo leer: sigue */ }
        }
      } catch (e) { todas = []; }
      const coinciden = todas.filter((n2) => n2.toLowerCase().indexOf(p.secuencia.toLowerCase()) !== -1);
      if (coinciden.length > 1) {
        throw new Error(
          `"${cmd}" se pidió sobre "${p.secuencia}" y eso coincide PARCIALMENTE con ` +
          `${coinciden.length} secuencias del proyecto: ${coinciden.map((x) => `"${x}"`).join(", ")}. ` +
          `La activa es "${nombre}", pero no hay forma de saber si es la que querías. ` +
          "NO se ejecutó nada: pasá el nombre completo."
        );
      }
    }
  }
