...
/*
 * SOLO LAS CLAVES DECLARADAS, Y LAS DEMAS REBOTAN.
 *
 * El panel tiene `PARAMS_DE`, que rechaza un parametro que el verbo no lee y dice cual acepta.
 * **Desde las herramientas MCP esa guarda era INALCANZABLE**: `inputSchema` se declaraba como un
 * shape de zod, y zod por DEFECTO descarta en silencio las claves que no estan en el schema. O
 * sea que la clave inventada nunca llegaba al panel y la guarda no la podia ver.
 *
 * Medido el 2026-09-21 con zod 4.4.3:
 *   z.object({nombre}).parse({nombre:"X", audio:true, activar:"V3"})  ->  {"nombre":"X"}
 *
 * Y no es teorico: en UNA sesion se cobro tres veces. `seleccionar` con `audio: true` selecciono
 * el clip de VIDEO e informo exito; `secuencias` con `activar` contesto "activa: VIDEO 3" como si
 * nada. Una de las tres llevo a un diagnostico equivocado. Es exactamente el mismo agujero que ya
 * tuvo la guarda `proyecto` —declarada en el despachador y no alcanzable desde las MCP— cobrado
 * de nuevo en el parametro de al lado.
 *
...
server.registerTool(
  "premiere_transicion",
  {
    title: "Poner un fundido en un corte",
    description:
      "Agrega una transicion de video en un CORTE de una pista. El default es el **Cross Dissolve " +
      "moderno** (`AE.AE_Impact_Dissolve`), que es el que Premiere pone con Cmd+D — no el Legacy.\n\n" +
      "Se apunta por TIEMPO: `segundos` es el corte, y el verbo resuelve a que clip y a que lado cayo " +
      "y lo informa. Si el corte mas cercano esta a mas de 1s, REBOTA sin hacer nada: un fundido en el " +
      "corte equivocado no tira error y solo se ve reproduciendo.\n\n" +
      "`alineacion` es una FRACCION 0..1 y esta medida: **0 arranca en el corte, 0,5 centrada (el " +
      "default), 1 termina en el corte**. OJO: NO son los `Constants.TransitionPosition` — ese enum " +
      "define START=1, y pasarlo da 'termina en el corte', el lado contrario al que sugiere el nombre.\n\n" +
      "**El AUDIO no se toca**, y se avisa en cada respuesta: la API de Premiere no expone transiciones " +
      "de audio (`TransitionFactory` solo tiene `createVideoTransition`), asi que el crossfade se hace " +
      "a mano. Un fundido de video sin el de audio es medio trabajo.\n\n" +
      "**El veredicto es un CONTEO.** Releer una transicion por API devuelve `null`, asi que el verbo " +
      "puede decir que aparecio una, no que este bien puesta. Para verificar donde quedo hay que abrir " +
      "el `.prproj` o mirar el timeline.\n\n" +
      "Un Cmd+Z la saca.",
    inputSchema: soloEstas({
      secuencia: z.string().optional().describe("Guarda: si la secuencia activa no es ésta, no se ejecuta nada."),
      proyecto: z
      ...
      pista: z.string().optional().describe('Pista, como "V1". Por defecto V1.'),
      segundos: z
        .number()
        .optional()
        .describe(
          "El CORTE donde va el fundido, en segundos de la secuencia. Es la forma natural de pedirlo. "
          + "Si no hay ningun corte a menos de 1s, rebota y lista los cortes que si hay."
        ),
      indice: z
        .number()
        .optional()
        .describe("Forma explicita, en vez de `segundos`: el clip. Se combina con `alInicio`."),
      alInicio: z
        .boolean()
        .optional()
        .describe("Con `indice`: true = la cabeza del clip (su corte de entrada), false = la cola."),
      alineacion: z
        .number()
        .optional()
        .describe("Fraccion 0..1. 0 arranca en el corte · 0,5 centrada (default) · 1 termina en el corte."),
      duracion: z.number().optional().describe("Largo en segundos. Default 1. Se cuantiza al cuadro."),
      matchName: z
        .string()
        .optional()
        .describe(
          "Otra transicion, por matchName. Si no existe entre las 164 que conoce Premiere, rebota y "
          + "ofrece las disolvencias disponibles."
        ),
    }),
  },
  async (args) => {
    try {
      const r = await enviar("transicion", args, 60000);
      return texto(r.resumen, r);
    } catch (e) {
      return fallo(e);
    }
...
server.registerTool(
  "premiere_limpiar_rangos",
  {
    title: "Sacar los in/out de la secuencia",
    description:
      "Saca los puntos de entrada y salida de la SECUENCIA — el Opt+X que hay que hacer a mano. " +
      "`premiere_estado` y `premiere_exportar` avisan cuando hay una marca vieja, porque " +
      "`exportSequence` la RESPETA y eso define el largo del archivo: un out olvidado deja el " +
      "export más largo, con negro y silencio al final, y eso no se nota, se entrega.\n\n" +
      "Hasta ahora ese aviso no tenía remedio por API. Sin marca los getters devuelven un " +
      "sentinel (-400000) y ese sentinel se puede reescribir, así que limpiar es escribirlo.\n\n" +
      "El veredicto sale de RELEER: si los dos getters no vuelven al sentinel, lo dice en vez " +
      "de darlo por bueno. Si la secuencia ya estaba limpia, contesta NO CAMBIÓ NADA y no " +
      "ejecuta ninguna transacción. Un Cmd+Z devuelve la marca.",
    inputSchema: soloEstas({
      secuencia: z.string().optional().describe("Guarda: si la secuencia activa no es ésta, no se ejecuta nada."),
      proyecto: z
        .string()
  ...
  async ({ proyecto, secuencia }) => {
    try {
      const r = await enviar("limpiarRangos", { proyecto, secuencia }, 120000);
      return { content: [{ type: "text", text: r.resumen }] };
    } catch (e) {
